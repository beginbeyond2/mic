...
public class TopViewHorizontalPicker extends View {
     ...
     */
    private static final int SELECTOR_ADJUSTMENT_DURATION_MILLIS = 400;
    ...
    private Paint bgPaint, bgPaintDefault;
    ...
    public TopViewHorizontalPicker(Context context) {
        this(context, null);
    }

    public TopViewHorizontalPicker(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.horizontalPickerStyle);
    }

    public TopViewHorizontalPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        bgPaintDefault = paint3;

        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.TopViewHorizontalPicker,
                defStyle, 0
        );
        ...
        try {
            textColor = a.getColorStateList(R.styleable.TopViewHorizontalPicker_android_textColor);
            if (textColor == null) {
                textColor = ColorStateList.valueOf(0xFF000000);
            }
            bgColor = a.getColorStateList(R.styleable.TopViewHorizontalPicker_bgColor);
            if (bgColor == null) {
                bgColor = ColorStateList.valueOf(0xFF000000);
            } else {
                bgPaintDefault.setColor(bgColor.getDefaultColor());
            }

            values = a.getTextArray(R.styleable.TopViewHorizontalPicker_values);
            ellipsize = a.getInt(R.styleable.TopViewHorizontalPicker_android_ellipsize, ellipsize);
            marqueeRepeatLimit = a.getInt(R.styleable.TopViewHorizontalPicker_android_marqueeRepeatLimit, marqueeRepeatLimit);
            dividerSize = a.getDimension(R.styleable.TopViewHorizontalPicker_dividerSize, dividerSize);
            sideItems = a.getInt(R.styleable.TopViewHorizontalPicker_sideItems, sideItems);

            float textSize = a.getDimension(R.styleable.TopViewHorizontalPicker_android_textSize, -1);
            if (textSize > -1) {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawRect(0, 0, getMeasuredWidth(), getMeasuredHeight(), bgPaintDefault);
        ...
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
                ...
                bgPaint.setColor(getBgColor(i));
                canvas.drawRect(clipBounds, bgPaint);
    ...
    private TextDirectionHeuristicCompat getTextDirectionHeuristic() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            // Always need to resolve topview direction first
    ...
    private int getTextColor(int item) {
        ...
        if (scrollX > itemWithPadding * item - itemWithPadding / 2 &&
            ...
            color = getColor(textColor, position, item);
    ...
    private int getBgColor(int item) {

        int scrollX = getScrollX();

        // set color of text
        int color = bgColor.getDefaultColor();
        int itemWithPadding = (int) (itemWidth + dividerSize);
        if (scrollX > itemWithPadding * item - itemWithPadding / 2 &&
                scrollX < itemWithPadding * (item + 1) - itemWithPadding / 2) {
            int position = scrollX - itemWithPadding / 2;
            color = getColor(bgColor, position, item);
        } else if (item == pressedItem) {
            color = bgColor.getColorForState(new int[]{android.R.attr.state_pressed}, color);
        }
...
//            postInvalidateOnAnimation(); // TODO
    ...
    private void calculateItemSize(int w, int h) {
        ...
        itemClipBounds = new RectF(0, 0 - getPaddingTop(), itemWidth, h);
     ...
     * @return
     */
    private int getColor(ColorStateList colorStateList, int scrollX, int position) {
        int itemWithPadding = (int) (itemWidth + dividerSize);
        ...
        if (pressedItem == position) {
            defaultColor = colorStateList.getColorForState(new int[]{android.R.attr.state_pressed}, textColor.getDefaultColor());
            selectedColor = colorStateList.getColorForState(new int[]{android.R.attr.state_pressed, android.R.attr.state_selected}, defaultColor);
        } else {
            defaultColor = colorStateList.getDefaultColor();
            selectedColor = colorStateList.getColorForState(new int[]{android.R.attr.state_selected}, defaultColor);
        }
        return (Integer) new ArgbEvaluator().evaluate(0, selectedColor, defaultColor);
    }
     ...
     * {@link TopViewHorizontalPicker#getInBoundsX(int)}
     *
    ...
    private static final class Marquee extends Handler {
        ...
        private final WeakReference<TopViewHorizontalPicker> mView;
        ...
        Marquee(TopViewHorizontalPicker v, Layout l, boolean rtl) {
            ...
            mView = new WeakReference<TopViewHorizontalPicker>(v);
        ...
        void tick() {
            ...
            final TopViewHorizontalPicker view = mView.get();
    ...
    private static class PickerTouchHelper extends ExploreByTouchHelper {
        ...
        private TopViewHorizontalPicker mPicker;

        public PickerTouchHelper(TopViewHorizontalPicker picker) {
            super(picker);
            mPicker = picker;
        }
