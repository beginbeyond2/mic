...
public abstract class BasePathSelectFragment extends AbstractFragment implements IFileShowFragment, IHandleFragment, ITabbarFragment, ITitlebarFragment {
     ...
     */
    public void close() {
        if (mConfigDataReference.get().buildController instanceof DialogController) {
            mConfigDataReference.get().buildController.getDialogFragment().dismiss();
        } else if (mConfigDataReference.get().buildController instanceof ActivityController) {
            mActivity.finish();
        } else if (mConfigDataReference.get().buildController instanceof FragmentController) {
            super.dismiss();
        }
    }
    ...
    public Dialog getDialog() {
        if (mConfigDataReference.get().buildController instanceof DialogController) {
            return mConfigDataReference.get().buildController.getDialogFragment().getDialog();
        }
        return null;
    }
