...
//                else {
//                    fileBean = new FileBean(parentPath, "", MConstants.FILEBEAN_BACK_FLAG);
...
//                }
    ...
    public void getFilesOnlyList(String currentPath, List<FileBean> fileBeanList, List<String> filterTypes) {
        ...
        for (File file : files) {
            if(file.isDirectory()){
                getFilesOnlyList(file.getPath(),fileBeanList,filterTypes);
            }else{
                String extension = FileUtils.getFileExtension(file);
                String [] parts = file.getPath().split("/");
                String innerPath = "";
                if(file.getPath().startsWith("/storage/emulated/0")){
                    innerPath = "/" + String.join("/", Arrays.copyOfRange(parts,4,parts.length));
                }else {
                    innerPath = "/" + String.join("/", Arrays.copyOfRange(parts,3,parts.length));
                }
                if(filterTypes.contains(extension)){
