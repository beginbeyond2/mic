...
public class TabbarListAdapter extends BaseQuickAdapter<TabbarFileBean, BaseViewHolder> implements LoadMoreModule {
    ...
    protected void convert(@NonNull BaseViewHolder holder, TabbarFileBean item) {
        ...
        ImageView iv = holder.getView(R.id.imgv_item_tabbar_mlh);
        int position = getItemPosition(item);
        int totalCount = getData().size();
        boolean isLastItem = (position == totalCount-1);
        if(isLastItem){
            tv.setText(item.getName());
            tv.setTextColor(getColor(R.color.select_bule));
            iv.setVisibility(View.GONE);
        }else{
            tv.setText(item.getName());
            iv.setVisibility(View.VISIBLE);
            tv.setTextColor(getColor(R.color.folder_text_color));
        }
