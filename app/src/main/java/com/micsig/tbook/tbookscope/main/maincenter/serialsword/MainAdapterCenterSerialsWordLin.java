...
public class MainAdapterCenterSerialsWordLin extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private ArrayList<SerialBusTxtStruct.LinStruct> list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new Holder(LayoutInflater.from(context).inflate(R.layout.item_serialsword_lin, parent, false));
    }
    ...
    class Holder extends RecyclerView.ViewHolder {
        private SerialsWordLinSingleRowTextView linTextView;

        public Holder(View itemView) {
            super(itemView);
            linTextView = (SerialsWordLinSingleRowTextView) itemView.findViewById(R.id.bean);
        }


        public void bind(SerialBusTxtStruct.LinStruct bean) {
            ViewGroup.LayoutParams layoutParams = linTextView.getLayoutParams();
            layoutParams.height = formHeightDetail * ((bean.Data.trim().length() - 1) / ISerialsWord.MAXCHAR_EACHROW_DATA_LIN + 1);
            linTextView.setLayoutParams(layoutParams);
            linTextView.setData(bean, showMs);
        }
    }
}
