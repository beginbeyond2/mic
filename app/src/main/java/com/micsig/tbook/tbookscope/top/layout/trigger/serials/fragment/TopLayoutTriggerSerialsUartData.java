...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位：UART串口触发Data详情Fragment                                          ║
 * ║  核心职责：配置UART协议Data触发条件（条件选择+数据编辑）                             ║
 * ║  架构设计：继承BaseDetail，实现UART Data的具体触发逻辑                             ║
 * ║  数据流向：右侧菜单 → setConsumer → 更新位宽/进制/数据 → sendMsg → FPGA命令        ║
 * ║  依赖关系：BaseDetail / UartBus / Command / CacheUtil / SerialsDetailUartData    ║
 * ║  使用场景：UART触发模式下Data子项的触发条件配置界面                                  ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailUartData; // 导入UART Data详情数据Bean
 ...
 *
 * UART串口触发Data详情Fragment。
 * 负责配置UART协议中Data字段的触发条件（条件选择+数据编辑），
 * 并将配置同步到FPGA命令和UartBus总线。
 */

public class TopLayoutTriggerSerialsUartData extends TopLayoutTriggerSerialsBaseDetail { // UART Data触发详情Fragment，继承基类
    private TopViewRadioGroup uartDataCondition; // Data条件选择单选组
    private TopViewEdit uartDataEdit; // Data数据编辑框
    private SerialsDetailUartData msgUartData; // Data详情数据Bean
    ...
    protected void initView(View view) { // 初始化界面控件
        uartDataCondition = (TopViewRadioGroup) view.findViewById(R.id.uartDataCondition); // 查找Data条件单选组控件
        uartDataEdit = (TopViewEdit) view.findViewById(R.id.uartDataEdit); // 查找Data数据编辑框控件
        uartDataCondition.setOnListener(onCheckChangedListener); // 设置条件单选组变更监听器
        uartDataEdit.setOnClickEditListener(onClickEditListener); // 设置数据编辑框点击监听器
        msgUartData = new SerialsDetailUartData(); // 创建Data详情数据Bean实例
        msgUartData.setUartDataCondition(uartDataCondition.getSelected()); // 设置初始条件选中项
        msgUartData.setUartDataConditionTitle(uartDataCondition.getHead()); // 设置条件标题
        msgUartData.setUartDataEdit(digits, uartDataEdit.getText()); // 设置初始数据编辑值
        msgUartData.setUartDataEditTitle(uartDataEdit.getHead()); // 设置数据编辑标题
    }

    /**
     * 获取布局资源ID。
     * @return UART Data布局资源ID
     */
    @Override // 重写基类的getLayoutResId抽象方法
    protected int getLayoutResId() { // 返回布局资源ID
        return R.layout.layout_triggerserials_uartdata; // 返回UART Data布局
    }

    /**
     * 获取串口详情数据，从缓存中读取最新的条件和数据值。
     * @param detailFlag 详情标志位
     * @return UART Data详情数据对象
     */
    @Override // 重写基类的getSerialsDetail抽象方法
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 获取串口详情数据
        int condition = CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_CONDITION + getSerialsNumber()); // 从缓存读取条件索引
        String edit = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_EDIT + getSerialsNumber()); // 从缓存读取数据编辑值
        if (!edit.equals(msgUartData.getUartDataEdit().getValue())) { // 如果编辑值与当前Bean不一致
            digits = CacheUtil.get().getInt(CacheUtil.RIGHT_SLIP_SERIALS_UART_DISPLAY + getSerialsNumber()) == 1 ? DIGITS_2 : DIGITS_16; // 根据右侧显示设置确定进制
            uartDataEdit.setText(edit); // 更新编辑框文本
            msgUartData.setUartDataEdit(digits, uartDataEdit.getText()); // 更新Bean中的编辑值
        }
        if (msgUartData.getUartDataCondition().getIndex() != condition) { // 如果条件索引与当前Bean不一致
            uartDataCondition.setSelectedIndex(condition); // 更新条件单选组选中项
            msgUartData.setUartDataCondition(uartDataCondition.getSelected()); // 更新Bean中的条件值
        }
        return msgUartData; // 返回详情数据Bean
    }
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 处理右侧菜单消息
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断是否为串口1且编号匹配
            ...
            if (uartDataEdit != null) { // 编辑框不为空时处理
                String edit = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_EDIT + getSerialsNumber()); // 从缓存读取编辑值
//                digits = CacheUtil.get().getInt(CacheUtil.RIGHT_SLIP_SERIALS_UART_DISPLAY + getSerialsNumber()) == 1 ? DIGITS_2 : DIGITS_16;
                uartDataEdit.setText(edit); // 更新编辑框文本
                msgUartData.setUartDataEdit(digits, uartDataEdit.getText()); // 更新Bean中的编辑值
                CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_EDIT + getSerialsNumber(), uartDataEdit.getText()); // 同步保存编辑值到缓存
                if (CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART + getSerialsNumber()) == 2) { // 判断UART子项是否选中Data
                    //当且仅当当前uart列表选中的是该项时，才向外发送消息
                    sendMsg(msgUartData, rightMsgSerials.isFromEventBus()); // 发送消息通知外部
                }
            }
        }
    }
    ...
    protected void setCache() { // 加载缓存数据
        ...
        uartDataCondition.setOnListener(null); // 临时移除监听器，避免加载时触发回调
        uartDataCondition.setSelectedIndex(condition); // 设置条件单选组选中项
        uartDataCondition.setOnListener(onCheckChangedListener); // 恢复监听器
        uartDataEdit.setText(edit); // 设置编辑框文本
        ...
        uartBus.setTriggerData(UartBus.UART_TRIGGER_DATA, toD(edit, digits)); // 设置Data触发数据

        msgUartData.setUartDataCondition(uartDataCondition.getSelected()); // 更新Bean条件值
        msgUartData.setUartDataEdit(digits, edit); // 更新Bean编辑值
        int triggerIndex = CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER); // 获取当前触发器索引
        if ( // 判断当前触发器是否匹配
                ((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber == CacheUtil.S1) // 判断S1是否匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART + getSerialsNumber()) == 2) { // 判断UART子项是否为Data
            sendMsg(msgUartData, false); // 发送消息通知外部
        }
    }
     ...
     */
    public void setCommandData(int conditionIndex, int data, boolean isFromEventBus) { // 设置命令数据
        String sData = SerialsUtils.getHexBinFromLong(data, digits == DIGITS_16 ? 2 : bits, digits); // 将数据转换为指定进制字符串
        if (!uartDataEdit.getText().equals(sData)) { // 如果编辑框文本与转换后数据不一致
            onTextListener(sData, isFromEventBus); // 触发文本变更处理
        }
        if (uartDataCondition.getSelected().getIndex() != conditionIndex) { // 如果条件索引不一致
            uartDataCondition.setSelectedIndex(conditionIndex); // 更新条件选中项
            onCheckListener(uartDataCondition, uartDataCondition.getSelected(), isFromEventBus); // 触发条件变更处理
        }
    }
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 编辑框点击处理
        ...
        switch (v.getId()) { // 根据编辑框ID分支处理
            case R.id.uartDataEdit: // Data编辑框
                //该为16进制，两位限制//2进制，右侧决定位数
                dialogKeyBoard.setDecimalData(digits == DIGITS_16 ? 2 : bits, digits, onDataListener); // 设置键盘参数：16进制2位或2进制按位宽
                break; // 跳出switch
        }
    }
    ...
    private void onTextListener(String text, boolean isFromEventBus) { // 文本变更处理
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_EDIT + getSerialsNumber(), text); // 保存编辑值到缓存
        uartDataEdit.setEdit(text); // 更新编辑框显示
        msgUartData.setUartDataEdit(digits, text); // 更新Bean编辑值
        sendMsg(msgUartData, isFromEventBus); // 发送消息通知外部
        Command.get().getTrigger_uart().setType(cmdCh, 2, uartDataCondition.getSelected().getIndex(), toD(text, digits), false); // 发送FPGA触发数据命令

        if (!isFromEventBus) { // 非EventBus事件时同步总线
            SerialChannel serialChannel = ChannelFactory.getSerialChannel(fpgaCh); // 获取串口通道实例
            if (serialChannel == null) return; // 通道为空则返回
            UartBus uartBus = (UartBus) serialChannel.getBus(IBus.UART); // 获取UART总线实例
            uartBus.setTriggerData(UartBus.UART_TRIGGER_DATA, toD(text, digits)); // 设置Data触发数据
        }
    }
    ...
    private void onCheckListener(TopViewRadioGroup view, TopBeanChannel item, boolean isFromEventBus) { // 条件变更处理
        int cmdCh=TChan.toFpgaChNo(serialsNumber); // 获取FPGA命令通道号
        int fpgaCh=TChan.toFpgaBySerialNumber(serialsNumber); // 获取FPGA总线通道号
        if (view.getId() == R.id.uartDataCondition) { // 判断是否为Data条件单选组
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_UART_DATA_CONDITION + getSerialsNumber(), String.valueOf(item.getIndex())); // 保存条件索引到缓存
            msgUartData.setUartDataCondition(item); // 更新Bean条件值
            sendMsg(msgUartData, isFromEventBus); // 发送消息通知外部
            Command.get().getTrigger_uart().setType(cmdCh, 2, item.getIndex(), toD(uartDataEdit.getText(), digits), false); // 发送FPGA触发条件命令
