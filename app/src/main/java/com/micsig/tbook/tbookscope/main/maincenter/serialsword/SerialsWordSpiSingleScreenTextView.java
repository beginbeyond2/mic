...
/**
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │                   SerialsWordSpiSingleScreenTextView                        │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【模块定位】                                                                │
 * │ SPI串口总线整屏文本显示视图，负责多个SPI数据帧的可视化呈现                    │
 * │                                                                             │
 * │ 【核心职责】                                                                │
 * │ 1. 显示多个SPI数据帧的通道号、时间戳、数据内容和触发标志                      │
 * │ 2. 处理多行数据的批量显示和换行逻辑                                          │
 * │ 3. 绘制分隔虚线以区分不同数据帧                                              │
 * │                                                                             │
 * │ 【架构设计】                                                                │
 ...
 * │ │  │ S1  │ 12.3ms   │ 0x04 0x05 0x06... │          │          │             │
 * │ │  └─────┴──────────┴───────────────────┴──────────┘          │             │
 * │ └──────────────────────────────────────────────────────────────┘             │
 * │                                                                             │
 * │ 【数据流向】                                                                │
 * │ ArrayList<SpiStruct> -> setData() -> invalidate() -> onDraw()               │
 * │                                                                             │
 * │ 【依赖关系】                                                                │
 ...
 * │                                                                             │
 * │ 【使用场景】                                                                │
 * │ 在串口解码功能中，用于显示整屏SPI数据帧列表，支持多行显示和自动换行            │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
public class SerialsWordSpiSingleScreenTextView extends View {
    ...
    private ArrayList<SerialBusTxtStruct.SpiStruct> list = new ArrayList<>(); // SPI数据列表
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0,drawLine = 0; i < list.size() && i < showLine; i++) { // 遍历数据列表
            SerialBusTxtStruct.SpiStruct bean = list.get(i);          // 获取当前数据帧
            ...
            int y = drawLine * rowHeight + rowHeight / 2 + chRect.height() / 2+offset; // 计算文本绘制Y坐标
            ...
            int curDrawLine = (data.length() - 1) / ISerialsWord.MAXCHAR_EACHROW_DATA_SPI + 1; // 计算当前数据需要绘制的行数
            for (int j = 0; j < curDrawLine; j++) {                  // 遍历每一行
                if (j != curDrawLine - 1) {                          // 非最后一行
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_SPI
                            ...
                            , chWidth + timeWidth + 15, y + j * rowHeight, paint); // 绘制完整行数据
                } else {                                              // 最后一行
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_SPI)
                            , chWidth + timeWidth + 15, y + j * rowHeight, paint); // 绘制剩余数据
