...
/*
 * ┌──────────────────────────────────────────────────────────────────────────────┐
 * │                     SerialsWordCanSingleScreenTextView                       │
 * ├──────────────────────────────────────────────────────────────────────────────┤
 * │ 【模块定位】                                                                  │
 * │   串行总线协议数据显示模块 - CAN总线单屏多行数据文本视图组件                     │
 * │                                                                              │
 * │ 【核心职责】                                                                  │
 * │   1. 显示CAN总线多帧数据的列表视图                                             │
 * │   2. 以表格形式批量展示多帧数据的通道、时间、ID、类型、DLC、数据、CRC、错误等信息│
 * │   3. 支持指定显示行数限制                                                     │
 * │   4. 支持虚线分隔符绘制                                                       │
 * │                                                                              │
 * │ 【架构设计】                                                                  │
 ...
 * │                                                                              │
 * │ 【数据流向】                                                                  │
 * │   ArrayList<CanStruct> → setData() → invalidate() → onDraw()                 │
 * │   FPGA采集 → 解析为CanStruct列表 → 视图批量渲染                               │
 * │                                                                              │
 * │ 【依赖关系】                                                                  │
 ...
 * │                                                                              │
 * │ 【使用场景】                                                                  │
 * │   用于CAN总线解码列表界面，在列表区域批量展示多帧CAN数据的概览信息              │
 * └──────────────────────────────────────────────────────────────────────────────┘
 */
public class SerialsWordCanSingleScreenTextView extends View {
    ...
    private ArrayList<SerialBusTxtStruct.CanStruct> list = new ArrayList<>();  // CAN总线数据列表
...
//            Logger.i(Command.TAG,"index:"+i);
            SerialBusTxtStruct.CanStruct bean = list.get(i);  // 获取当前帧数据对象
            ...
            for (int j = 0; j < curDrawLine; j++) {  // 循环绘制每一行数据
                if (j != curDrawLine - 1) {  // 如果不是最后一行（中间行）
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_CAN
                            ...
                            , chWidth + timeWidth + idWidth + typeWidth + dlcWidth + 15, y + j * rowHeight, paint);  // 绘制中间行数据片段
                } else {  // 如果是最后一行
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_CAN)
                            , chWidth + timeWidth + idWidth + typeWidth + dlcWidth + 15, y + j * rowHeight, paint);  // 绘制最后一行剩余数据
