package com.micsig.tbook.tbookscope.top.layout.trigger.serials.fragment; // ARINC429 SSM触发条件Fragment所在包
...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailArinc429Ssm; // 导入ARINC429 SSM数据模型
...
/**
 * ┌─────────────────────────────────────────────────────────────────────┐
 * │            TopLayoutTriggerSerialsArinc429Ssm                      │
 * │              ARINC429 SSM 触发条件配置Fragment                      │
 * ├─────────────────────────────────────────────────────────────────────┤
 * │ 模块定位：top.layout.trigger.serials.fragment → 串口触发子页面      │
 * │ 核心职责：配置ARINC429总线的SSM(符号/状态矩阵)触发条件              │
 * │ 架构设计：继承BaseDetail，复用通用触发框架，仅实现差异化逻辑         │
 * │ 数据流向：UI输入→CacheUtil持久化→Command发送FPGA→ARINC429Bus同步    │
 * │ 依赖关系：SerialsDetailArinc429Ssm(数据模型)、                     │
 * │           TopDialogNumberKeyBoard(数字键盘)、CacheUtil(持久化)       │
 * │ 使用场景：Micsig示波器ARINC429协议解码时，按SSM触发捕获             │
 * ├─────────────────────────────────────────────────────────────────────┤
 * │ 触发类型索引：RightLayoutSerials.SERIALS_M429 子项5                 │
...
public class TopLayoutTriggerSerialsArinc429Ssm extends TopLayoutTriggerSerialsBaseDetail { // ARINC429 SSM触发条件Fragment，继承串口触发基类
    private TopViewEdit arinc429SsmLabel; // SSM输入框，2进制2位
    private SerialsDetailArinc429Ssm msgArinc429Ssm; // ARINC429 SSM触发数据模型
    ...
    protected void initView(View view) { // 初始化视图方法，由基类onCreateView调用
        arinc429SsmLabel = (TopViewEdit) view.findViewById(R.id.arinc429SsmLabel); // 查找SSM输入框控件
        arinc429SsmLabel.setOnClickEditListener(onClickEditListener); // 设置SSM输入框点击监听，弹出数字键盘
        msgArinc429Ssm = new SerialsDetailArinc429Ssm(); // 创建ARINC429 SSM数据模型实例
        msgArinc429Ssm.setArinc429SsmLabel(IDigits.DIGITS_2, arinc429SsmLabel.getText()); // 初始化SSM字段（2进制，当前显示值）
        msgArinc429Ssm.setArinc429SsmLabelTitle(arinc429SsmLabel.getHead()); // 设置SSM字段标题（如"SSM"）
    } // initView方法结束
    ...
    protected int getLayoutResId() { // 获取Fragment布局资源ID的方法
        return R.layout.layout_triggerserials_arinc429ssm; // 返回ARINC429 SSM触发布局
    } // getLayoutResId方法结束

    /**
     * 获取串口触发详情数据
     * @param detailFlag 详情标志位
     * @return ARINC429 SSM触发数据
     */
    @Override
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 获取当前触发详情数据的方法
        String ssm = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M429_SSM + getSerialsNumber()); // 从缓存读取SSM值
        msgArinc429Ssm.setArinc429SsmLabel(IDigits.DIGITS_2, ssm); // 更新模型中的SSM字段
        return msgArinc429Ssm; // 返回填充后的数据模型
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 消费右侧串口消息的方法
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断：是串口1的消息且当前Fragment对应串口1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M429 + getSerialsNumber()) == 5) { // 且429子项选中的是第5项(SSM)
            //当且仅当当前429列表选中的是该项时，才向外发送消息
            sendMsg(msgArinc429Ssm, rightMsgSerials.isFromEventBus()); // 条件全部满足，发送触发数据消息
        } // if条件判断结束
    } // setConsumer方法结束
    ...
    protected void setCache() { // 从缓存恢复数据并同步硬件的方法
        String ssm = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M429_SSM + getSerialsNumber()); // 读取缓存的SSM值
        arinc429SsmLabel.setText(ssm); // 设置SSM输入框显示值
        ...
        a429Bus.setSSM(ARINC429Bus.ARINC429_TRIGGER_SSM, toD(ssm, IDigits.DIGITS_2)); // 设置ARINC429总线的SSM触发值（2进制转10进制）

        msgArinc429Ssm.setArinc429SsmLabel(IDigits.DIGITS_2, ssm); // 同步更新数据模型中的SSM
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断：触发页是串口1且当前Fragment对应串口1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M429 + getSerialsNumber()) == 5) { // 且429子项选中第5项(SSM)
            sendMsg(msgArinc429Ssm, false); // 当前页面处于激活状态，发送触发数据消息
        } // if条件判断结束
    } // setCache方法结束

    /**
     * 通过指令数据设置SSM值（用于EventBus远程同步）
     * @param ssm SSM值（整数）
     * @param isFromEventBus 是否来自EventBus远程事件
     */
    public void setCommandData(int ssm, boolean isFromEventBus) { // 设置指令数据的方法，用于远程同步
        String sSsm = SerialsUtils.getHexBinFromLong(ssm, 2, DIGITS_2); // 将SSM整数值转为2进制2位字符串
        if (!arinc429SsmLabel.getText().equals(sSsm)) { // 如果SSM值与当前显示不同
            onTextListener(sSsm, isFromEventBus); // 触发SSM变更监听，更新UI和硬件
        } // SSM变更判断结束
    } // setCommandData方法结束
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 输入框点击回调方法
        ...
        switch (v.getId()) { // 根据点击的控件ID分发
            case R.id.arinc429SsmLabel: // 点击SSM输入框
                //该为2进制,2位限制
    ...
    private void onTextListener(String text, boolean isFromEventBus) { // 文本变更核心处理方法
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M429_SSM + getSerialsNumber(), text); // 持久化SSM值到缓存
        arinc429SsmLabel.setEdit(text); // 更新SSM输入框显示
        msgArinc429Ssm.setArinc429SsmLabel(IDigits.DIGITS_2, text); // 更新数据模型中的SSM
        sendMsg(msgArinc429Ssm, isFromEventBus); // 发送触发数据消息
        Command.get().getTrigger_m429().setType(cmdCh, 5, 0, 0, 0, toD(text, IDigits.DIGITS_2), false); // 发送FPGA触发指令：类型5=SSM，传入SSM的十进制值
        ...
        if (!isFromEventBus) { // 非EventBus来源时同步硬件总线
            ...
            a429Bus.setSSM(ARINC429Bus.ARINC429_TRIGGER_SSM, toD(text, IDigits.DIGITS_2)); // 设置总线SSM触发值（2进制转10进制）
        } // 硬件同步结束
    } // onTextListener方法结束
} // TopLayoutTriggerSerialsArinc429Ssm类结束
