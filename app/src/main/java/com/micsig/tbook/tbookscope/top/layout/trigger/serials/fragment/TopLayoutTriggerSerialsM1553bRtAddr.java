...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailM1553bRtAddr; // 导入M1553B远程终端地址触发详情Bean
...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位: 顶部触发布局 → 串口触发 → M1553B子条件 → 远程终端地址触发Fragment  ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  核心职责:                                                                 ║
 * ║    1. 提供MIL-STD-1553B远程终端(RT)地址触发的UI交互界面（RT地址输入）          ║
 * ║    2. 支持2进制/16进制显示模式切换                                           ║
 * ║    3. 管理RT地址的缓存读写与FPGA指令下发                                     ║
 * ║    4. 将用户输入同步到MILSTD1553BBus总线和Command命令通道                     ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  架构设计:                                                                 ║
 * ║    继承TopLayoutTriggerSerialsBaseDetail基类，实现模板方法模式                ║
 * ║    - initView(): 初始化RT地址编辑框和消息Bean                                ║
 * ║    - setCache(): 从缓存恢复状态并同步到FPGA                                  ║
 * ║    - setConsumer(): 响应右侧菜单消息，条件匹配时发送触发消息                  ║
 * ║    - onTextListener(): 统一处理文本变更，更新缓存/Bean/FPGA                   ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  数据流向:                                                                 ║
 * ║    UI编辑框 → CacheUtil缓存 → SerialsDetailM1553bRtAddr Bean               ║
 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  使用场景:                                                                 ║
 * ║    当用户在M1553B触发模式下选择"远程终端地址"子条件时，加载此Fragment          ║
 * ║    对应M1553B子条件索引值为3，RT地址最大值为1F(5位2进制/2位16进制)             ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */

public class TopLayoutTriggerSerialsM1553bRtAddr extends TopLayoutTriggerSerialsBaseDetail { // M1553B远程终端地址触发Fragment，继承串口触发详情基类
    private SerialsDetailM1553bRtAddr msgM1553bRtAddr; // M1553B远程终端地址触发详情消息Bean
    private TopViewEdit m1553bRtAddrRtAddr; // 远程终端地址编辑框
    ...
    protected void initView(View view) { // 重写基类方法，初始化视图控件
        m1553bRtAddrRtAddr = (TopViewEdit) view.findViewById(R.id.m1553bRtAddrRtAddr); // 查找RT地址编辑框控件
        m1553bRtAddrRtAddr.setOnClickEditListener(onClickEditListener); // 为RT地址编辑框设置点击监听
        msgM1553bRtAddr = new SerialsDetailM1553bRtAddr(); // 创建M1553B RT地址消息Bean实例
        msgM1553bRtAddr.setM1553bRtAddrRtAddr(digits, m1553bRtAddrRtAddr.getText()); // 设置RT地址值（使用当前进制）
        msgM1553bRtAddr.setM1553bRtAddrRtAddrTitle(m1553bRtAddrRtAddr.getHead()); // 设置RT地址字段标题
    }

    /**
     * 获取布局资源ID
     * @return M1553B RT地址布局资源ID
     */
    @Override
    protected int getLayoutResId() { // 重写基类方法，返回布局资源ID
        return R.layout.layout_triggerserials_m1553brtaddr; // 返回M1553B RT地址布局
    }

    /**
     * 获取串口触发详情数据
     * @param detailFlag 详情标志位
     * @return M1553B RT地址触发详情Bean
     */
    @Override
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 重写基类方法，获取串口触发详情
        String rtAddr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M1553B_RTADDR + getSerialsNumber()); // 从缓存读取RT地址值
        ...
        msgM1553bRtAddr.setM1553bRtAddrRtAddr(digits, rtAddr); // 设置RT地址到消息Bean
        return msgM1553bRtAddr; // 返回消息Bean
    }
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 重写基类方法，处理右侧菜单消息
        if (serialsNumber == rightMsgSerials.getSerialsNumber() // 判断串口序号是否匹配
            ...
            if (m1553bRtAddrRtAddr != null) { // 编辑框已初始化
                String rtAddr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M1553B_RTADDR + getSerialsNumber()); // 从缓存读取RT地址
                m1553bRtAddrRtAddr.setText(rtAddr); // 更新编辑框文本
                msgM1553bRtAddr.setM1553bRtAddrRtAddr(digits, m1553bRtAddrRtAddr.getText()); // 更新消息Bean RT地址
                if (CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M1553B + getSerialsNumber()) == 3) { // 判断M1553B子条件是否为RT地址(索引3)
                    //当且仅当当前1554b列表选中的是该项时，才向外发送消息
                    sendMsg(msgM1553bRtAddr, rightMsgSerials.isFromEventBus()); // 发送M1553B RT地址触发消息
                }
            }
        }
    }
    ...
    protected void setCache() { // 重写基类方法，从缓存恢复数据
        ...
        m1553bRtAddrRtAddr.setText(rtAddr); // 设置RT地址编辑框文本
        ...
        m1553bBus.setAddr(toD(rtAddr, digits)); // 设置M1553B RT地址

        msgM1553bRtAddr.setM1553bRtAddrRtAddr(digits, rtAddr); // 同步RT地址到消息Bean
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断是否为S1触发详情且当前为S1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M1553B + getSerialsNumber()) == 3) { // 判断M1553B子条件是否为RT地址
            sendMsg(msgM1553bRtAddr, false); // 发送触发消息（非EventBus来源）
        }
    }
     ...
     */
    public void setCommandData(int rtAddr, boolean isFromEventBus) { // 外部命令设置触发数据
        String sRtAddr = SerialsUtils.getHexBinFromLong(rtAddr, digits == DIGITS_16 ? 2 : 5, digits); // 将RT地址转为对应进制字符串（16进制2位或2进制5位）
        if (!m1553bRtAddrRtAddr.getText().equals(sRtAddr)) { // RT地址值有变化时
            onTextListener(sRtAddr, isFromEventBus); // 触发文本变更监听
        }
    }
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 重写基类方法，编辑框点击监听
        ...
        switch (v.getId()) { // 根据编辑框ID分发
            case R.id.m1553bRtAddrRtAddr: // RT地址编辑框
                //该为2进制,5位限制//16进制，2位，最大1F
                dialogKeyBoard.setDecimalData(digits == DIGITS_16 ? 2 : 5, digits, onRtAddrListener); // 根据进制设置键盘位数（16进制2位或2进制5位），绑定RT地址监听
                break;
        }
    }

    /** RT地址键盘关闭监听，将结果传递给文本变更监听 */
    private TopDialogNumberKeyBoard.OnDismissListener onRtAddrListener = new TopDialogNumberKeyBoard.OnDismissListener() { // RT地址键盘关闭回调
        @Override
        public void onDismiss(String result) { // 键盘关闭时回调
            onTextListener(result, false); // 触发RT地址文本变更（非EventBus来源）
        }
    };
    ...
    private void onTextListener(String text, boolean isFromEventBus) { // 文本变更统一处理方法
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_M1553B_RTADDR + getSerialsNumber(), text); // 缓存RT地址值
        m1553bRtAddrRtAddr.setEdit(text); // 更新RT地址编辑框显示
        msgM1553bRtAddr.setM1553bRtAddrRtAddr(digits, text); // 更新消息Bean RT地址
        sendMsg(msgM1553bRtAddr, isFromEventBus); // 发送触发消息
        Command.get().getTrigger_m1553B().setType(cmdCh, 3, 0, toD(text, digits), 0, false); // 下发M1553B触发指令到FPGA（类型3=RT地址）
        ...
        if (!isFromEventBus) { // 非EventBus来源时同步到软件解码总线
            ...
            m1553bBus.setAddr(toD(text, digits)); // 设置M1553B RT地址
        }
    }
}
