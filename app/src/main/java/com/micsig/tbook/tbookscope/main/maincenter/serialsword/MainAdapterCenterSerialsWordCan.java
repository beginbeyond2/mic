 ...
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【模块定位】                                                                │
 * │   RecyclerView适配器，负责将CAN总线解码数据绑定到列表视图，实现数据可视化展示    │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【核心职责】                                                                │
 * │   1. 管理CAN总线数据列表的显示与更新                                        │
 ...
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【数据流向】                                                                │
 * │   SerialBusTxtStruct.CanStruct数据列表 → Adapter → Holder →                │
 * │   SerialsWordCanSingleRowTextView（单行视图组件）                          │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【依赖关系】                                                                │
 * │   依赖：SerialBusTxtStruct.CanStruct（CAN数据结构）                        │
 * │   依赖：SerialsWordCanSingleRowTextView（CAN单行显示视图）                  │
 * │   依赖：ISerialsWord（常量定义）                                           │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 【使用场景】                                                                │
 * │   在串行总线解码列表界面中，用于显示CAN协议的解码结果，每行显示一个CAN帧的       │
 * │   时间戳、ID、数据等信息                                                    │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
public class MainAdapterCenterSerialsWordCan extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private ArrayList<SerialBusTxtStruct.CanStruct> list; // CAN总线数据列表，存储所有待显示的CAN帧数据
     ...
     * @param list CAN总线数据列表，包含所有待显示的CAN帧数据
     */
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new Holder(LayoutInflater.from(context).inflate(R.layout.item_serialsword_can, parent, false)); // 加载CAN单行布局并创建Holder
    }
    ...
    class Holder extends RecyclerView.ViewHolder {
        private SerialsWordCanSingleRowTextView canTextView; // CAN单行数据显示视图组件
        ...
        public Holder(View itemView) {
            ...
            canTextView = (SerialsWordCanSingleRowTextView) itemView.findViewById(R.id.bean); // 从布局中获取CAN数据视图组件
        }
        ...
        public void bind(SerialBusTxtStruct.CanStruct bean) {
            ViewGroup.LayoutParams layoutParams = canTextView.getLayoutParams(); // 获取视图布局参数
            layoutParams.height = formHeightDetail * ((bean.Data.trim().length() - 1) / ISerialsWord.MAXCHAR_EACHROW_DATA_CAN + 1); // 根据数据长度计算行高：数据字符数除以每行最大字符数，向上取整后乘以单行高度
            canTextView.setLayoutParams(layoutParams); // 应用新的布局参数
            canTextView.setData(bean, showMs); // 设置CAN数据到视图组件，传入毫秒显示标志
        }
    }
}