 ...
 * *
 * * 【模块定位】
 * *   主界面中心区域 - SPI总线协议解码文本数据的显示模块
 * *
 * * 【核心职责】
 * *   1. 显示SPI总线解码后的文本数据列表（数据、时间戳等）
 ...
 * *   - 通过LinkedBlockingQueue获取线程安全的SPI数据队列
 * *   - 利用RxBus订阅缓存加载事件
 * *   - 采用自适应滚动条显示数据位置
 * *
 * * 【数据流向】
 * *   输入：
 * *     → SerialBusManage获取SPI数据队列和缓冲区
 * *     → CacheUtil读取运行状态和通道类型
 * *     → RxBus接收LoadCache缓存加载事件
 * *     → 用户触摸事件触发滚动
 * *   输出：
 * *     → RecyclerView/SingleScreenTextView显示SPI文本列表
 * *     → SerialBusManage清空缓冲区
 * *
 * * 【依赖关系】
 * *   上层依赖：MainLayoutCenterSerialsWordDetail父容器Fragment
 * *   下层依赖：MainAdapterCenterSerialsWordSpi适配器、SerialBusManage数据管理
 * *   平级依赖：CacheUtil、RxBus、PlaySound、Tools等工具类
 * *
 * * 【使用场景】
 * *   当用户配置通道为SPI协议时，显示SPI总线解码的文本数据
...
public class MainLayoutCenterSerialsWordSpi extends Fragment implements SerialBusManage.IForceRefreshUI {
    private Button btnClear;  // 清空按钮，用于清除所有SPI数据缓存
    ...
    private MainAdapterCenterSerialsWordSpi allScreenAdapter;  // SPI数据适配器，绑定数据到视图
    private ArrayList<SerialBusTxtStruct.SpiStruct> allScreenList;  // 全屏数据列表，存储所有SPI帧数据
    private View singleScreenScrollBar;  // 单屏模式滚动条视图，显示数据位置指示
    private SerialsWordSpiSingleScreenTextView singleScreenView;  // 单屏数据视图，用于Run模式高效显示
    private ArrayList<SerialBusTxtStruct.SpiStruct> singleScreenList;  // 单屏数据列表，存储当前显示的SPI帧数据
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.layout_maincenter_serialsword_spi, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        singleScreenView = (SerialsWordSpiSingleScreenTextView) view.findViewById(R.id.singleScreenTextView);
        ...
        allScreenAdapter = new MainAdapterCenterSerialsWordSpi(getContext(), allScreenList);
    ...
    public void onForceRefreshUI() {
        LinkedBlockingQueue<SerialBusTxtStruct.SpiStruct> spiListTotal = SerialBusManage.getInstance()
                .getSerialTxtBufferQueue(chType, RightLayoutSerials.SERIALS_SPI,false);
        SerialTxtBuffer serialTxtBuffer = SerialBusManage.getInstance().getSerialTxtBuffer(chType);
        updateLastList(spiListTotal, serialTxtBuffer, false);
    }
     ...
     */
    private void updateLastList(LinkedBlockingQueue<SerialBusTxtStruct.SpiStruct> spiListTotal, SerialTxtBuffer serialTxtBuffer, boolean isAll) {
        if (spiListTotal == null) return;
        allScreenList.clear();
        int showLine = 20;
        int listSize ;
        if (chType==ISerialsWord.TYPE_S12)listSize=SerialTxtBuffer.getSpiS1S2CurrScreenSize();
        else listSize = serialTxtBuffer.getSpiCurrScreenSize();
        if (!isAll && listSize >= showLine) {
            ...
            for (Iterator iter = spiListTotal.iterator(); iter.hasNext(); ) {
                ...
                if (i >= (listSize - showLine)) {
                    singleScreenList.add((SerialBusTxtStruct.SpiStruct) iter.next());
                } else {
                    iter.next();
                }
                i++;
...
//            int totalSize = listSize;
            int totalSize=chType==ISerialsWord.TYPE_S12?SerialTxtBuffer.getSpiS1S2CurrSize():serialTxtBuffer.getSpiCurrSize();
            ...
            allScreenList.addAll(spiListTotal);
    ...
    public void setRunStop(boolean run) {
        ...
        LinkedBlockingQueue<SerialBusTxtStruct.SpiStruct> spiListTotal = SerialBusManage.getInstance()
                .getSerialTxtBufferQueue(chType, RightLayoutSerials.SERIALS_SPI,!run);
        SerialTxtBuffer serialTxtBuffer = SerialBusManage.getInstance().getSerialTxtBuffer(chType);
        updateLastList(spiListTotal, serialTxtBuffer, !run);
    }
