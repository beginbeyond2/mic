...
/*
 * ┌──────────────────────────────────────────────────────────────────────────────┐
 * │                     SerialsWordM429SingleScreenTextView                      │
 * ├──────────────────────────────────────────────────────────────────────────────┤
 * │ 【模块定位】                                                                  │
 * │   串行总线协议数据显示模块 - ARINC429总线单屏多行数据文本视图组件               │
 * │                                                                              │
 * │ 【核心职责】                                                                  │
 * │   1. 显示ARINC429总线多帧数据的列表视图                                       │
 * │   2. 以表格形式批量展示多帧数据的通道、时间、Label、SDI、数据、SSM、错误、触发│
 * │   3. 支持指定显示行数限制                                                     │
 * │   4. 支持虚线分隔符绘制                                                       │
 * │   5. Label字段使用八进制编码显示                                              │
 * │                                                                              │
 ...
 * │                                                                              │
 * │ 【数据流向】                                                                  │
 * │   ArrayList<Arinc429Struct> → setData() → invalidate() → onDraw()            │
 * │   FPGA采集 → 解析为Arinc429Struct列表 → Label八进制转换 → 视图批量渲染        │
 * │                                                                              │
 * │ 【依赖关系】                                                                  │
 ...
 * │                                                                              │
 * │ 【使用场景】                                                                  │
 * │   用于ARINC429总线解码列表界面，在列表区域批量展示多帧ARINC429数据的概览信息  │
 * │   Label字段以八进制格式显示                                                   │
 * └──────────────────────────────────────────────────────────────────────────────┘
 */
public class SerialsWordM429SingleScreenTextView extends View {
    ...
    private ArrayList<SerialBusTxtStruct.Arinc429Struct> list = new ArrayList<>();  // ARINC429总线数据列表
    ...
    /**
     * 设置数据方法
     * 接收ARINC429总线数据列表并触发视图重绘
     * 
     * @param list ARINC429总线数据列表，包含多帧数据
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, drawLine = 0; i < list.size() && i < showLine; i++) {  // 循环遍历数据列表，限制显示行数
            SerialBusTxtStruct.Arinc429Struct bean = list.get(i);  // 获取当前帧数据对象
            ...
            for (int j = 0; j < curDrawLine; j++) {  // 循环绘制每一行数据
                if (j != curDrawLine - 1) {  // 如果不是最后一行（中间行）
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_429
                            ...
                            , chWidth + timeWidth + labelWidth + sdiWidth + 15, y + j * rowHeight, paint);  // 绘制中间行数据片段
                } else {  // 如果是最后一行
                    canvas.drawText(data.substring(j * ISerialsWord.MAXCHAR_EACHROW_DATA_429)
                            , chWidth + timeWidth + labelWidth + sdiWidth + 15, y + j * rowHeight, paint);  // 绘制最后一行剩余数据
