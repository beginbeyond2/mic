...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailI2cNoAckInAdr; // 导入I2C地址无应答触发详情Bean
...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位: 顶部触发布局 → 串口触发 → I2C子条件 → 地址无应答触发Fragment        ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  核心职责:                                                                 ║
 * ║    1. 提供I2C地址无应答(No ACK)触发的UI交互界面（地址数据输入）                ║
 * ║    2. 管理地址数据的缓存读写与FPGA指令下发                                   ║
 * ║    3. 将用户输入同步到I2CBus总线和Command命令通道                             ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  架构设计:                                                                 ║
 * ║    继承TopLayoutTriggerSerialsBaseDetail基类，实现模板方法模式                ║
 * ║    - initView(): 初始化数据编辑框和消息Bean                                  ║
 * ║    - setCache(): 从缓存恢复状态并同步到FPGA                                  ║
 * ║    - setConsumer(): 响应右侧菜单消息，条件匹配时发送触发消息                  ║
 * ║    - onTextListener(): 统一处理文本变更，更新缓存/Bean/FPGA                   ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  数据流向:                                                                 ║
 * ║    UI编辑框 → CacheUtil缓存 → SerialsDetailI2cNoAckInAdr Bean              ║
 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  使用场景:                                                                 ║
 * ║    当用户在I2C触发模式下选择"地址无应答"子条件时，加载此Fragment               ║
 * ║    对应I2C子条件索引值为4                                                   ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */

public class TopLayoutTriggerSerialsI2cNoAckInAdr extends TopLayoutTriggerSerialsBaseDetail { // I2C地址无应答触发Fragment，继承串口触发详情基类
    private TopViewEdit i2cNoAckInAdrData; // 地址无应答数据编辑框
    private SerialsDetailI2cNoAckInAdr msgI2cNoAckInAdr; // I2C地址无应答触发详情消息Bean
    ...
    protected void initView(View view) { // 重写基类方法，初始化视图控件
        i2cNoAckInAdrData = (TopViewEdit) view.findViewById(R.id.i2cNoAckInAdrData); // 查找数据编辑框控件
        i2cNoAckInAdrData.setOnClickEditListener(onClickEditListener); // 为数据编辑框设置点击监听
        msgI2cNoAckInAdr = new SerialsDetailI2cNoAckInAdr(); // 创建I2C地址无应答消息Bean实例
        msgI2cNoAckInAdr.setI2cNoAckInAdrData(DIGITS_16, i2cNoAckInAdrData.getText()); // 设置数据值（16进制）
        msgI2cNoAckInAdr.setI2cNoAckInAdrDataTitle(i2cNoAckInAdrData.getHead()); // 设置数据字段标题
    }

    /**
     * 获取布局资源ID
     * @return I2C地址无应答布局资源ID
     */
    @Override
    protected int getLayoutResId() { // 重写基类方法，返回布局资源ID
        return R.layout.layout_triggerserials_i2cnoackinadr; // 返回I2C地址无应答布局
    }

    /**
     * 获取串口触发详情数据
     * @param detailFlag 详情标志位
     * @return I2C地址无应答触发详情Bean
     */
    @Override
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 重写基类方法，获取串口触发详情
        String noAckInAdr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_NOACKINADR + getSerialsNumber()); // 从缓存读取地址无应答数据
        msgI2cNoAckInAdr.setI2cNoAckInAdrData(DIGITS_16, noAckInAdr); // 设置数据到消息Bean（16进制）
        return msgI2cNoAckInAdr; // 返回消息Bean
    }
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 重写基类方法，处理右侧菜单消息
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断是否为串口1且当前序号匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C + getSerialsNumber()) == 4) { // 判断I2C子条件是否为地址无应答(索引4)
            //当且仅当当前i2c列表选中的是该项时，才向外发送消息
            sendMsg(msgI2cNoAckInAdr, rightMsgSerials.isFromEventBus()); // 发送I2C地址无应答触发消息
        }
    }
    ...
    protected void setCache() { // 重写基类方法，从缓存恢复数据
        String noAckInAdr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_NOACKINADR + getSerialsNumber()); // 从缓存读取地址无应答数据
        i2cNoAckInAdrData.setText(noAckInAdr); // 设置数据编辑框文本
        ...
        i2cBus.setTriggerAddrs(I2CBus.I2C_TRIGGER_ADDRESS_NO_ACK, toD(noAckInAdr, IDigits.DIGITS_16)); // 设置I2C触发地址（地址无应答模式）

        msgI2cNoAckInAdr.setI2cNoAckInAdrData(DIGITS_16, noAckInAdr); // 同步数据到消息Bean
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断是否为S1触发详情且当前为S1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C + getSerialsNumber()) == 4) { // 判断I2C子条件是否为地址无应答
            sendMsg(msgI2cNoAckInAdr, false); // 发送触发消息（非EventBus来源）
        }
    }
     ...
     */
    public void setCommandData(int addr, boolean isFromEventBus) { // 外部命令设置触发数据
        String sAddr = SerialsUtils.getHexBinFromLong(addr, 2, DIGITS_16); // 将地址转为2位16进制字符串
        if (!i2cNoAckInAdrData.getText().equals(sAddr)) { // 地址值有变化时
            onTextListener(sAddr, isFromEventBus); // 触发文本变更监听
        }
    }
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 重写基类方法，编辑框点击监听
        ...
        switch (v.getId()) { // 根据编辑框ID分发
            case R.id.i2cNoAckInAdrData: // 数据编辑框
                //该为16进制,2位限制
    ...
    private TopDialogNumberKeyBoard.OnDismissListener onDataListener = new TopDialogNumberKeyBoard.OnDismissListener() { // 数据键盘关闭回调
        ...
        public void onDismiss(String result) { // 键盘关闭时回调
            onTextListener(result, false); // 触发文本变更（非EventBus来源）
        }
    };
    ...
    private void onTextListener(String text, boolean isFromEventBus) { // 文本变更统一处理方法
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_NOACKINADR + getSerialsNumber(), text); // 缓存地址无应答数据
        i2cNoAckInAdrData.setEdit(text); // 更新数据编辑框显示
        msgI2cNoAckInAdr.setI2cNoAckInAdrData(DIGITS_16, text); // 更新消息Bean数据
        sendMsg(msgI2cNoAckInAdr, isFromEventBus); // 发送触发消息
        int cmdCh=TChan.toFpgaChNo(serialsNumber); // 获取FPGA命令通道号
        int fpgaCh=TChan.toFpgaBySerialNumber(serialsNumber); // 获取FPGA物理通道号
        Command.get().getTrigger_iic().setType(cmdCh, 4, toD(text, IDigits.DIGITS_16), 0, 0, 0, false); // 下发I2C触发指令到FPGA（类型4=地址无应答）
        ...
        if (!isFromEventBus) { // 非EventBus来源时同步到软件解码总线
            ...
            i2cBus.setTriggerAddrs(I2CBus.I2C_TRIGGER_ADDRESS_NO_ACK, toD(text, IDigits.DIGITS_16)); // 设置I2C触发地址（地址无应答模式）
        }
    }
}
