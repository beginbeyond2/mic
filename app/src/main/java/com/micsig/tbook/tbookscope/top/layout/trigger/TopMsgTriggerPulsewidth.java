 ...
 * ║                   脉宽触发器配置消息数据类                                     ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  模块名称: TopMsgTriggerPulsewidth                                            ║
 * ║  所属模块: 触发器配置模块                                                      ║
 * ║  创建时间: 2017/5/17                                                         ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  核心职责:                                                                    ║
 * ║  1. 封装脉宽触发器的触发源/极性/条件/脉宽/时间上下限参数                       ║
 ...
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
public class TopMsgTriggerPulsewidth implements ITriggerDetail { // 脉宽触发器配置消息数据类 //
    /** 触发源参数 */ // 触发源 //
    ...
    /** 脉宽值 */ // 脉宽 //
    private RxStringWithSelect pulsewidth; // 脉宽值 //
    /** 时间上限详情 */ // 时间上限 //
    ...
    public Object clone() throws CloneNotSupportedException { // 克隆方法 //
        TopMsgTriggerPulsewidth topMsgTriggerPulsewidth = (TopMsgTriggerPulsewidth) super.clone(); // 浅拷贝当前对象 //
        topMsgTriggerPulsewidth.triggerSource = (TopBeanChannel) topMsgTriggerPulsewidth.triggerSource.clone(); // 深拷贝触发源 //
        topMsgTriggerPulsewidth.polar = (TopBeanChannel) topMsgTriggerPulsewidth.polar.clone(); // 深拷贝极性 //
        topMsgTriggerPulsewidth.condition = (TopBeanChannel) topMsgTriggerPulsewidth.condition.clone(); // 深拷贝条件 //
        topMsgTriggerPulsewidth.pulsewidth = (RxStringWithSelect) topMsgTriggerPulsewidth.pulsewidth.clone(); // 深拷贝脉宽 //
        topMsgTriggerPulsewidth.timeHighDetail = (RxStringWithSelect) topMsgTriggerPulsewidth.timeHighDetail.clone(); // 深拷贝时间上限 //
        topMsgTriggerPulsewidth.timeLowDetail = (RxStringWithSelect) topMsgTriggerPulsewidth.timeLowDetail.clone(); // 深拷贝时间下限 //
        return topMsgTriggerPulsewidth; // 返回深拷贝对象 //
    } // 方法结束 //
    ...
    /**
     * 获取脉宽值
     * @return RxStringWithSelect 脉宽值
     */
    public RxStringWithSelect getPulsewidth() { // 获取脉宽 //
        return pulsewidth; // 返回脉宽值 //
    } // 方法结束 //

    /**
     * 设置脉宽值
     * @param pulsewidth 脉宽字符串
     */
    public void setPulsewidth(String pulsewidth) { // 设置脉宽 //
        if (this.pulsewidth == null) { // 如果脉宽为空（首次设置） //
            this.pulsewidth = new RxStringWithSelect(pulsewidth); // 创建新对象 //
        } else { // 如果脉宽已存在 //
            this.pulsewidth.setValue(pulsewidth); // 更新值 //
            setAllUnSelect(); // 清除所有RxMsg选中状态 //
            this.pulsewidth.setRxMsgSelect(true); // 标记脉宽为选中 //
        } // 判断结束 //
    } // 方法结束 //
    ...
    private void setAllUnSelect() { // 清除所有选中状态 //
        ...
        pulsewidth.setRxMsgSelect(false); // 清除脉宽选中 //
    } // 方法结束 //

    @Override // 重写toString //
    public String toString() { // 转字符串方法 //
        return "TopMsgTriggerPulsewidth{" + // 返回类名 //
                "triggerSource=" + triggerSource + // 包含触发源 //
                ", polar=" + polar + // 包含极性 //
                ", condition=" + condition + // 包含条件 //
                ", pulsewidth='" + pulsewidth + // 包含脉宽 //
