...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailI2cFrame1; // 导入I2C帧1触发详情Bean
...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位: 顶部触发布局 → 串口触发 → I2C子条件 → 帧1触发Fragment              ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  核心职责:                                                                 ║
 * ║    1. 提供I2C帧1触发的UI交互界面（地址+数据输入）                              ║
 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  数据流向:                                                                 ║
 * ║    UI编辑框 → CacheUtil缓存 → SerialsDetailI2cFrame1 Bean                  ║
 ...
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  使用场景:                                                                 ║
 * ║    当用户在I2C触发模式下选择"帧1"子条件时，加载此Fragment                     ║
 * ║    对应I2C子条件索引值为5                                                   ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */

public class TopLayoutTriggerSerialsI2cFrame1 extends TopLayoutTriggerSerialsBaseDetail { // I2C帧1触发Fragment，继承串口触发详情基类
    private TopViewEdit i2cFrame1Addr; // 帧1地址编辑框
    private TopViewEdit i2cFrame1Data; // 帧1数据编辑框
    private SerialsDetailI2cFrame1 msgI2cFrame1; // I2C帧1触发详情消息Bean
    ...
    protected void initView(View view) { // 重写基类方法，初始化视图控件
        i2cFrame1Addr = (TopViewEdit) view.findViewById(R.id.i2cFrame1Addr); // 查找地址编辑框控件
        i2cFrame1Data = (TopViewEdit) view.findViewById(R.id.i2cFrame1Data); // 查找数据编辑框控件
        i2cFrame1Addr.setOnClickEditListener(onClickEditListener); // 为地址编辑框设置点击监听
        i2cFrame1Data.setOnClickEditListener(onClickEditListener); // 为数据编辑框设置点击监听
        msgI2cFrame1 = new SerialsDetailI2cFrame1(); // 创建I2C帧1消息Bean实例
        msgI2cFrame1.setI2cFrame1Addr(DIGITS_16, i2cFrame1Addr.getText()); // 设置地址值（16进制）
        msgI2cFrame1.setI2cFrame1Data(DIGITS_16, i2cFrame1Data.getText()); // 设置数据值（16进制）
        msgI2cFrame1.setI2cFrame1AddrTitle(i2cFrame1Addr.getHead()); // 设置地址字段标题
        msgI2cFrame1.setI2cFrame1DataTitle(i2cFrame1Data.getHead()); // 设置数据字段标题
    }
    ...
    protected int getLayoutResId() { // 重写基类方法，返回布局资源ID
        return R.layout.layout_triggerserials_i2cframe1; // 返回I2C帧1布局
    }

    /**
     * 获取串口触发详情数据
     * @param detailFlag 详情标志位
     * @return I2C帧1触发详情Bean
     */
    @Override
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 重写基类方法，获取串口触发详情
        String addr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_ADDR + getSerialsNumber()); // 从缓存读取地址值
        String data = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_DATA + getSerialsNumber()); // 从缓存读取数据值
        msgI2cFrame1.setI2cFrame1Addr(DIGITS_16, addr); // 设置地址到消息Bean（16进制）
        msgI2cFrame1.setI2cFrame1Data(DIGITS_16, data); // 设置数据到消息Bean（16进制）
        return msgI2cFrame1; // 返回消息Bean
    }
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 重写基类方法，处理右侧菜单消息
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断是否为串口1且当前序号匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C + getSerialsNumber()) == 5) { // 判断I2C子条件是否为帧1(索引5)
            //当且仅当当前i2c列表选中的是该项时，才向外发送消息
            sendMsg(msgI2cFrame1, rightMsgSerials.isFromEventBus()); // 发送I2C帧1触发消息
        }
    }
    ...
    protected void setCache() { // 重写基类方法，从缓存恢复数据
        String addr = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_ADDR + getSerialsNumber()); // 从缓存读取地址
        String data = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_DATA + getSerialsNumber()); // 从缓存读取数据
        i2cFrame1Addr.setText(addr); // 设置地址编辑框文本
        i2cFrame1Data.setText(data); // 设置数据编辑框文本
        ...
        i2cBus.setTriggerAddrs(I2CBus.I2C_TRIGGER_FRAME1, toD(addr, DIGITS_16)); // 设置I2C触发地址（帧1模式）
        i2cBus.setTriggerData1(I2CBus.I2C_TRIGGER_FRAME1, toD(data, DIGITS_16)); // 设置I2C触发数据1（帧1模式）

        msgI2cFrame1.setI2cFrame1Addr(DIGITS_16, addr); // 同步地址到消息Bean
        msgI2cFrame1.setI2cFrame1Data(DIGITS_16, data); // 同步数据到消息Bean
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断是否为S1触发详情且当前为S1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C + getSerialsNumber()) == 5) { // 判断I2C子条件是否为帧1
            sendMsg(msgI2cFrame1, false); // 发送触发消息（非EventBus来源）
        }
    }
    ...
    public void setCommandData(int addr, int data1, boolean isFromEventBus) { // 外部命令设置触发数据
        ...
        if (!i2cFrame1Addr.getText().equals(sAddr)) { // 地址值有变化时
            onTextListener(i2cFrame1Addr, sAddr, isFromEventBus); // 触发地址变更监听
        }
        if (!i2cFrame1Data.getText().equals(sData1)) { // 数据值有变化时
            onTextListener(i2cFrame1Data, sData1, isFromEventBus); // 触发数据变更监听
        }
    }
    ...
    private TopDialogNumberKeyBoard.OnDismissListener onAddrListener = new TopDialogNumberKeyBoard.OnDismissListener() { // 地址键盘关闭回调
        ...
        public void onDismiss(String result) { // 键盘关闭时回调
            onTextListener(i2cFrame1Addr, result, false); // 触发地址文本变更（非EventBus来源）
        }
    };
    ...
    private TopDialogNumberKeyBoard.OnDismissListener onDataListener = new TopDialogNumberKeyBoard.OnDismissListener() { // 数据键盘关闭回调
        @Override
        public void onDismiss(String result) { // 键盘关闭时回调
            onTextListener(i2cFrame1Data, result, false); // 触发数据文本变更（非EventBus来源）
        }
    };
    ...
    private void onTextListener(TopViewEdit view, String result, boolean isFromEventBus) { // 文本变更统一处理方法
        ...
        if (view.getId() == i2cFrame1Addr.getId()) { // 判断是否为地址编辑框
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_ADDR + getSerialsNumber(), result); // 缓存地址值
            i2cFrame1Addr.setEdit(result); // 更新地址编辑框显示
            msgI2cFrame1.setI2cFrame1Addr(DIGITS_16, result); // 更新消息Bean地址
            sendMsg(msgI2cFrame1, isFromEventBus); // 发送触发消息
            Command.get().getTrigger_iic().setType(cmdCh, 5, toD(i2cFrame1Addr.getText(), IDigits.DIGITS_16), toD(i2cFrame1Data.getText(), IDigits.DIGITS_16), 0, 0, false); // 下发I2C触发指令到FPGA（类型5=帧1）
            ...
            if (!isFromEventBus) { // 非EventBus来源时同步到软件解码总线
                ...
                i2cBus.setTriggerAddrs(I2CBus.I2C_TRIGGER_FRAME1, toD(result, IDigits.DIGITS_16)); // 设置I2C触发地址（帧1模式）
            }
        } else if (view.getId() == i2cFrame1Data.getId()) { // 判断是否为数据编辑框
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_I2C_FRAME1_DATA + getSerialsNumber(), result); // 缓存数据值
            i2cFrame1Data.setEdit(result); // 更新数据编辑框显示
            msgI2cFrame1.setI2cFrame1Data(DIGITS_16, result); // 更新消息Bean数据
            sendMsg(msgI2cFrame1, isFromEventBus); // 发送触发消息
            Command.get().getTrigger_iic().setType(cmdCh, 5, toD(i2cFrame1Addr.getText(), IDigits.DIGITS_16), toD(i2cFrame1Data.getText(), IDigits.DIGITS_16), 0, 0, false); // 下发I2C触发指令到FPGA（类型5=帧1）
            ...
            if (!isFromEventBus) { // 非EventBus来源时同步到软件解码总线
                ...
                if(serialChannel == null) return; // 通道为空则返回
                I2CBus i2cBus = (I2CBus) serialChannel.getBus(IBus.I2C); // 获取I2C总线
                i2cBus.setTriggerData1(I2CBus.I2C_TRIGGER_FRAME1, toD(result, IDigits.DIGITS_16)); // 设置I2C触发数据1（帧1模式）
            }
        }
    }
}
