...
/**
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │ 模块定位：保存/调用功能 → 顶部布局 → 消息对象 → 保存总消息（Save Message）    │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 核心职责：作为保存功能的顶层消息载体，封装当前选中的保存标题、保存详情和         │
 * │          事件来源标识，用于在RxBus事件总线中传递保存状态信息                    │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 架构设计：POJO消息类，包含TopAllBeanTitle（标题选中状态）、                   │
 * │          ISaveDetail（保存详情接口）和isFromEventBus（事件来源标识），          │
 * │          通过RxBus在TopLayoutSave与子页面之间传递                             │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 数据流向：子页面(状态变化) → TopMsgSave(封装消息) →                          │
 * │          RxBus.TOPLAYOUT_SAVE(事件发送) → TopLayoutSave(接收处理)             │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 依赖组件：TopAllBeanTitle, ISaveDetail                                       │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 使用场景：当用户在保存子页面（Wav/Csv/Bin等）操作时，                          │
 * │          子页面通过此消息对象将当前状态传递给父级TopLayoutSave                  │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
public class TopMsgSave {
