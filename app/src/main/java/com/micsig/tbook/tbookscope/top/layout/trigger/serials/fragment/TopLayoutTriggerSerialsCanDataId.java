...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位：CAN串口触发DataId详情Fragment                                         ║
 * ║  核心职责：配置CAN协议Data帧ID触发条件（ID编辑）                                    ║
 * ║  架构设计：继承BaseDetail，实现CAN DataId的具体触发逻辑                             ║
 * ║  数据流向：右侧菜单 → setConsumer → 更新ID → sendMsg → FPGA命令                   ║
 * ║  依赖关系：BaseDetail / CanBus / Command / CacheUtil / SerialsDetailCanDataId   ║
 * ║  使用场景：CAN触发模式下Data帧ID子项的触发条件配置界面                                ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailCanDataId; // 导入CAN DataId详情数据Bean
 ...
 *
 * CAN串口触发DataId详情Fragment。
 * 负责配置CAN协议中Data帧ID的触发条件（16进制ID编辑），
 * 并将配置同步到FPGA命令和CanBus总线。
 */

public class TopLayoutTriggerSerialsCanDataId extends TopLayoutTriggerSerialsBaseDetail { // CAN DataId触发详情Fragment，继承基类
    private TopViewEdit canDataIdEdit; // CAN Data帧ID编辑框
    private SerialsDetailCanDataId msgCanDataId; // CAN DataId详情数据Bean
    ...
    protected void initView(View view) { // 初始化界面控件
        canDataIdEdit = (TopViewEdit) view.findViewById(R.id.canDataIdEdit); // 查找CAN Data帧ID编辑框控件
        canDataIdEdit.setOnClickEditListener(onClickEditListener); // 设置编辑框点击监听器
        msgCanDataId = new SerialsDetailCanDataId(); // 创建CAN DataId详情数据Bean实例
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, canDataIdEdit.getText()); // 设置初始ID值（16进制）
        msgCanDataId.setCanDataIdEditTitle(canDataIdEdit.getHead()); // 设置ID编辑标题
    }

    /**
     * 获取布局资源ID。
     * @return CAN DataId布局资源ID
     */
    @Override // 重写基类的getLayoutResId抽象方法
    protected int getLayoutResId() { // 返回布局资源ID
        return R.layout.layout_triggerserials_candataid; // 返回CAN DataId布局
    }

    /**
     * 获取串口详情数据，从缓存中读取最新的ID值。
     * @param detailFlag 详情标志位
     * @return CAN DataId详情数据对象
     */
    @Override // 重写基类的getSerialsDetail抽象方法
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 获取串口详情数据
        String dataId = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_DATAID + getSerialsNumber()); // 从缓存读取ID值
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, dataId); // 更新Bean中的ID值
        return msgCanDataId; // 返回详情数据Bean
    }

    /**
     * 处理右侧菜单串口消息，当CAN类型且选中DataId时发送消息。
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 处理右侧菜单消息
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断是否为串口1且编号匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN + getSerialsNumber()) == 2) { // 判断CAN子项是否选中DataId
            //当且仅当当前can列表选中的是该项时，才向外发送消息
            sendMsg(msgCanDataId, rightMsgSerials.isFromEventBus()); // 发送消息通知外部
        }
    }
    ...
    protected void setCache() { // 加载缓存数据
        String dataId = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_DATAID + getSerialsNumber()); // 从缓存读取ID值
        canDataIdEdit.setText(dataId); // 设置编辑框文本
        ...
        canBus.setFrameId(CanBus.CAN_TRIGGER_DATA_FRAME_ID, toD(dataId, IDigits.DIGITS_16)); // 设置CAN Data帧ID

        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, dataId); // 更新Bean ID值
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断S1是否匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN + getSerialsNumber()) == 2) { // 判断CAN子项是否为DataId
            sendMsg(msgCanDataId, false); // 发送消息通知外部
        }
    }
    ...
    public void setCommandData(int id, boolean isFromEventBus) { // 设置命令数据
        ...
        if (!canDataIdEdit.getText().equals(sId)) { // 如果编辑框文本与转换后ID不一致
            onTextListener(sId, isFromEventBus); // 触发文本变更处理
        }
    }

    /**
     * 处理单选组条件变更事件（CAN DataId无条件选择，空实现）。
     * @param view 单选组控件
     * @param item 选中的通道项
     */
    @Override // 重写基类的setOnCheckChangedListener抽象方法
    protected void setOnCheckChangedListener(TopViewRadioGroup view, TopBeanChannel item) { // 条件变更处理（CAN DataId无条件选择）
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 编辑框点击处理
        ...
        switch (v.getId()) { // 根据编辑框ID分支处理
            case R.id.canDataIdEdit: // CAN Data帧ID编辑框
                //该为16进制,8位限制
    ...
    private void onTextListener(String text, boolean isFromEventBus) { // 文本变更处理
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_DATAID + getSerialsNumber(), text); // 保存ID到缓存
        canDataIdEdit.setEdit(text); // 更新编辑框显示
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, text); // 更新Bean ID值
        sendMsg(msgCanDataId, isFromEventBus); // 发送消息通知外部
        Command.get().getTrigger_can().setType(cmdCh, 2, toD(text, IDigits.DIGITS_16), 0, 0, false); // 发送FPGA CAN DataId触发命令

        if (!isFromEventBus) { // 非EventBus事件时同步总线
            SerialChannel serialChannel = ChannelFactory.getSerialChannel(fpgaCh); // 获取串口通道实例
            if(serialChannel == null) return; // 通道为空则返回
            CanBus canBus = (CanBus) serialChannel.getBus(IBus.CAN); // 获取CAN总线实例
            canBus.setFrameId(CanBus.CAN_TRIGGER_DATA_FRAME_ID, toD(text, IDigits.DIGITS_16)); // 设置CAN Data帧ID
        }
    }
}
