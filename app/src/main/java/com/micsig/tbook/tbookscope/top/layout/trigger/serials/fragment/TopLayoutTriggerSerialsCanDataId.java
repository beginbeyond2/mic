...
public class TopLayoutTriggerSerialsCanDataId extends TopLayoutTriggerSerialsBaseDetail {
    ...
    protected void initView(View view) {
        canDataIdEdit = (TopViewEdit) view.findViewById(R.id.canDataIdEdit);
        canDataIdEdit.setOnClickEditListener(onClickEditListener);
        msgCanDataId = new SerialsDetailCanDataId();
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, canDataIdEdit.getText());
        msgCanDataId.setCanDataIdEditTitle(canDataIdEdit.getHead());
    }
    ...
    public ISerialsDetail getSerialsDetail(int detailFlag) {
        ...
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, dataId);
        return msgCanDataId;
    }
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) {
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN + getSerialsNumber()) == 2) {
            //当且仅当当前can列表选中的是该项时，才向外发送消息
            sendMsg(msgCanDataId, rightMsgSerials.isFromEventBus());
        }
    }
    ...
    protected void setCache() {
        ...
        canBus.setFrameId(CanBus.CAN_TRIGGER_DATA_FRAME_ID, toD(dataId, IDigits.DIGITS_16));
    ...
    private void onTextListener(String text, boolean isFromEventBus) {
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_DATAID + getSerialsNumber(), text);
        canDataIdEdit.setEdit(text);
        msgCanDataId.setCanDataIdEdit(IDigits.DIGITS_16, text);
        sendMsg(msgCanDataId, isFromEventBus);
        Command.get().getTrigger_can().setType(cmdCh, 2, toD(text, IDigits.DIGITS_16), 0, 0, false);

        if (!isFromEventBus) {
            SerialChannel serialChannel = ChannelFactory.getSerialChannel(fpgaCh);
            if(serialChannel == null) return;
            CanBus canBus = (CanBus) serialChannel.getBus(IBus.CAN);
            canBus.setFrameId(CanBus.CAN_TRIGGER_DATA_FRAME_ID, toD(text, IDigits.DIGITS_16));
        }
    }
}
