...
/**
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 * │ 模块定位：保存/调用功能 → 顶部布局 → CSV文件保存（Save Csv）                  │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 核心职责：提供CSV数据文件的保存功能界面，用户可选择多个通道、设置文件名、         │
 * │          选择保存路径，执行CSV格式波形数据的保存操作                            │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 架构设计：继承Fragment，通过TopViewChannel选择保存通道（支持多选），            │
 * │          通过TopViewSpinner选择保存路径，通过SaveManage执行保存，              │
 * │          通过RxBus监听通道状态、命令等事件，通过EventObserver监听保存进度       │
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 数据流向：用户选择(多通道/路径/文件名) → SaveManage.allSaveEntrance() →      │
 ...
 * ├─────────────────────────────────────────────────────────────────────────────┤
 * │ 使用场景：用户在Store页面选择CSV保存类型时，配置通道、路径和文件名后保存         │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
public class TopLayoutSaveCsv extends Fragment {
    /** 日志标签 */
    private static final String TAG = "TopLayoutSaveCsv";
    /** 保存位置常量：本地存储 */
     ...
     * @return 填充后的CSV保存布局视图
     */
    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.layout_save_csv, container, false); // 填充CSV保存布局
    }
    ...
    private void initView(View view) {
        viewChannel = view.findViewById(R.id.chanSaveCsv); // 获取通道选择视图
    ...
    public void savePathToCache() {
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_CSV, // 保存显示名称路径列表（CSV类型）
        ...
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_CSV, // 保存绝对路径列表（CSV类型）
        ...
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV, spinner.getSelectItem()); // 保存当前选中的路径
    }
...
//        RxBus.getInstance().getObservable(RxEnum.MAINBOTTOM_QUICKSAVE).subscribe(consumerMainBottomQuickSave);
        EventFactory.addEventObserver(EventFactory.EVENT_SAVECSV_RUN, eventSaveCsvObserver); // 注册CSV保存进度事件观察者
...
//        setChannelShow();
    ...
    private void setCache() {
        ...
        String waveName = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_CSV); // 获取缓存的文件名
...
//        Command.get().getStorage().Save_Type(1, false);
//        Command.get().getStorage().Save_ALLSegments(false, false);
        ...
        TChan.foreachCh1ToR8(chan -> { // 遍历所有通道
            if (TChan.isMath(chan)) { // 如果是Math通道
                ...
                boolean isAddByUser = CacheUtil.get().getBoolean(CacheUtil.RIGHT_SLIP_ADD_BY_USER_MATH + chan); // 获取是否用户添加
                channelShow[TChan.toFpgaChNo(chan)] = isCheck && isAddByUser; // CSV支持Math通道
            }
            if (TChan.isRef(chan)) { // 如果是Ref通道
                boolean isCheck = CacheUtil.get().getBoolean(CacheUtil.RIGHT_SLIP_REF_CHECK + chan); // 获取Ref选中状态
                boolean isAddByUser = CacheUtil.get().getBoolean(CacheUtil.RIGHT_SLIP_ADD_BY_USER_REF + chan); // 获取是否用户添加
                channelShow[TChan.toFpgaChNo(chan)] = isCheck && isAddByUser; // CSV支持Ref通道
            }
        });
        ...
        boolean isFileNumAddCheck = CacheUtil.get().getOtherBoolean(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_CSV); // 获取序号添加开关状态
        String suffixNum = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV); // 获取缓存的序号
    ...
    private void restorePath() {
        ...
        String pathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_CSV); // 获取显示名称路径缓存
        String abPathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_CSV); // 获取绝对路径缓存
        String currentPath = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV); // 获取当前选中路径
...
//        }
        ...
        for (int i = 0; i < abPathCacheList.size(); i++) { // 遍历绝对路径列表
            ...
            Logger.i(TAG, "abPath= " + abPathCacheList.get(i) + " ,disPath= " + pathCacheList.get(i) + " ,currnt= " + currentPath); // 记录路径日志
            if(abPathCacheList.get(i).equals(currentPath)) { // 如果是当前选中的路径
    ...
    private Consumer<RightMsgRefForEight> consumerRightRef = new Consumer<RightMsgRefForEight>() { // 创建消费者
        ...
        public void accept(RightMsgRefForEight msgRef) throws Exception { // 接收到Ref变化事件
            //哪个通道变化 设置哪个通道
            boolean isAddByUser = CacheUtil.get().getBoolean(CacheUtil.RIGHT_SLIP_ADD_BY_USER_REF + msgRef.getRefChannelNumber()); // 获取是否用户添加
            channelShow[TChan.toFpgaChNo(msgRef.getRefChannelNumber())] = msgRef.getRefChecked().isValue() && isAddByUser; // 更新Ref通道可见性
            setChannelShow(); // 更新通道显示
        }
    };
...
//            Command.get().getStorage().Save_Source(checkedIndex, false);
    ...
    TopViewChannelMultipleChoice.onTestListener onChannelItemTestListener = new TopViewChannelMultipleChoice.onTestListener() { // 创建监听器
        ...
        public void onTest(CheckBox checkBox) { // 多选测试回调
            List<Integer> lisSelects = viewChannel.getViewChannelMultipleChoice().getSelectChannel(); // 获取多选通道列表
            boolean showBin = true; // BIN类型是否可用标志
            for (Integer chanIdx : lisSelects) { // 遍历选中通道
                if (ChannelFactory.isMathCh(chanIdx) || ChannelFactory.isRefCh(chanIdx)) { // 如果包含Math或Ref通道
                    showBin = false; // BIN不可用
                    break; // 跳出循环
                }
            }
        }
...
//                        Command.get().getStorage().Save_Filename(saveNameEdit.getText().toString(), false);
                        ...
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_CSV, result); // 缓存文件名
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV, "000"); // 缓存序号
                    }
                });
            }
        }
    ...
    public void channelShowChange() {
        viewChannel.setAllSelectShow(true);//csv时显示多选控件 // CSV模式显示多选控件
        setChannelShow(); // 更新通道显示
    }
    ...
    private CompoundButton.OnCheckedChangeListener onCheckBoxChangedListener = new CompoundButton.OnCheckedChangeListener() { // 创建监听器
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) { // 选中状态变化回调
            if (buttonView.getId() == checkFileNameAdd.getId()) { // 文件名添加序号复选框
                CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_CSV, String.valueOf(isChecked)); // 缓存状态
                txtSuffixNum.setEnabled(isChecked); // 设置序号文本可用性
                RxBus.getInstance().post(RxEnum.MQ_MSG_SAVE_OR_INVOKE_SUFFIX_NUM_UPDATE, CacheUtil.WAVE_TYPE_CSV + CacheUtil.WAVE_STORE_PATH_SLIP + isChecked); // 通知序号更新
            }
        }
    };
     ...
     * 按钮点击事件监听器
     */
    private final View.OnClickListener onClickListener = new View.OnClickListener() { // 创建监听器
        @Override
        public void onClick(View v) { // 点击回调
            PlaySound.getInstance().playButton(); // 播放按键音效
            if (v.getId() == btnSave.getId()) { // 保存按钮
                String finalInput = getFinaleName(); // 获取最终文件名
                String filePath = spinner.getSelectItem() + File.separator + finalInput + ".csv"; // 构造完整文件路径
    ...
    private void doSaveWave(String filePath) {
        ...
        List<Integer> selectList = viewChannel.getAllSelectChannel();//保存CSV时选中的channel // 获取CSV保存选中的通道列表

        if (selectList == null || selectList.size() <= 0) { // 如果没有选中任何通道
            DToast.get().show(R.string.msgTopSaveCsvNotSelect); // 提示未选中通道
            return; // 返回
        }
        ...
        SaveManage.getInstance().allSaveEntrance(ch, 1, spinner.getSelectItem(), finalInput, selectList, new SaveManage.SaveCallBack() { // 调用保存入口（类型1=CSV）
            @Override
...
//            String oldSuffix = "_" + txtSuffixNum.getText().toString().trim();
//            String oldFilename = saveNameEdit.getText();
//            boolean isEndWithSuffix = oldFilename.endsWith(oldSuffix);
//            String tempFileName = oldFilename;
//            if (isEndWithSuffix) {
//                tempFileName = oldFilename.substring(0, oldFilename.length() - oldSuffix.length());
//            }
//            txtSuffixNum.setText(text);
//            saveNameEdit.setText(tempFileName + "_" + text);
//        } else {
//            txtSuffixNum.setText(text);
//        }
        txtSuffixNum.setText(text); // 设置序号文本
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV, text); // 缓存序号
    }
    ...
    TopViewSpinner.onItemSelectListener onItemSelectListener = new TopViewSpinner.onItemSelectListener() { // 创建监听器
        ...
        public void onItemSelected(FileBean str) { // 选中项变化回调
            CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV, str.getPath()); // 缓存当前路径
     ...
     * CSV保存进度事件的UI观察者
     */
    EventUIObserver eventSaveCsvObserver = new EventUIObserver() { // 创建观察者
        @Override
        public void update(Object data) { // 接收到事件更新
            ...
            switch (base.getId()) { // 根据事件ID分发
                case EventFactory.EVENT_SAVECSV_RUN: // CSV保存运行事件
