...
public class TopLayoutSaveBin extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.layout_save_bin, container, false);
    }
    ...
    private void initView(View view) {
        viewChannel = view.findViewById(R.id.chanSaveBin);
        viewChannel.setChangeListener(onChannelItemClickListener, onChannelItemTestListener);
        cbAllSegments = view.findViewById(R.id.allSegments);
        ...
        cbAllSegments.setOnCheckedChangeListener(onCheckBoxChangedListener);
    ...
    public void savePathToCache() {
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_BIN,
                StrUtil.getStringFromList(FileBeanToStr.getDisPlayStrList(pathSet.getPositiveList()), CacheUtil.WAVE_STORE_PATH_SLIP));

        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_BIN,
                StrUtil.getStringFromList(FileBeanToStr.getAbsoluteStrList(pathSet.getPositiveList()), CacheUtil.WAVE_STORE_PATH_SLIP));
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_BIN, spinner.getSelectItem());
    }
...
//        RxBus.getInstance().getObservable(RxEnum.MAINBOTTOM_QUICKSAVE).subscribe(consumerMainBottomQuickSave);
        RxBus.getInstance().getObservable(RxEnum.TOPLAYOUT_SAMPLESEGMENTED).subscribe(consumerTopSampleSegmented);
...
//        setChannelShow();
    ...
    private void setCache() {
        ...
        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_SAVE_ALL_SEGMENT_CHECK, String.valueOf(false), true);
        cbAllSegments.setChecked(false);
        cbAllSegments.setTextColor(getResources().getColor(R.color.textColorNewTopViewEnable));

        String waveName = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_BIN);
        if (waveName.isEmpty()) {
...
//        Command.get().getStorage().Save_Filename(saveNameEdit.getText().toString(), false);
        Command.get().getStorage().Save_Type(2, false);
...
//                channelShow[TChan.toFpgaChNo(chan)] = isCheck && isAddByUser;
                channelShow[TChan.toFpgaChNo(chan)] = false;//bin不需要显示Math通道
            }
            if (TChan.isRef(chan)) {
...
//                channelShow[TChan.toFpgaChNo(chan)] = isCheck && isAddByUser;
                channelShow[TChan.toFpgaChNo(chan)] = false;//bin不需要显示Ref通道
            }
        });
        setChannelShow();
        ...
        boolean isFileNumAddCheck = CacheUtil.get().getOtherBoolean(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_BIN);
        String suffixNum = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_BIN);
    ...
    private void restorePath() {
        pathSet.clear();
        String pathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_BIN);
        String abPathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_BIN);

        String currentPath = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_BIN);
...
//        }
        ...
        for (int i = 0; i < abPathCacheList.size(); i++) {
            ...
            if(pathCacheList.get(i).equals(currentPath)) {
...
//            channelShow[TChan.toFpgaChNo(msgRef.getRefChannelNumber())] = msgRef.getRefChecked().isValue() && isAddByUser;
            channelShow[TChan.toFpgaChNo(msgRef.getRefChannelNumber())] = false;
            setChannelShow();
        }
    };
    ...
    private Consumer<CommandMsgToUI> consumerCommandToUI = new Consumer<CommandMsgToUI>() {
        ...
        public void accept(CommandMsgToUI commandMsgToUI) throws Exception {
            switch (commandMsgToUI.getFlag()) {
                ...
                break;
                case CommandMsgToUI.FLAG_STOTAGE_SAVE_ALLSEGMENTS: {
                    boolean b = Boolean.parseBoolean(commandMsgToUI.getParam());
                    cbAllSegments.setChecked(b);
                    onCheckBoxChangedListener.onCheckedChanged(cbAllSegments, b);
                }
                break;
            }
        }
...
//        }
//    };
    private Consumer<TopMsgSampleSegmented> consumerTopSampleSegmented = new Consumer<TopMsgSampleSegmented>() {
        @Override
        public void accept(TopMsgSampleSegmented msgSegmented) throws Exception {
            setAllSegmentsVisible();
            if (!msgSegmented.getState().isValue()) {
                cbAllSegments.setChecked(false);
            }
        }
    };
    ...
    private boolean setAllSegmentsVisible() {
        ...
        int segmentState = CacheUtil.get().getInt(CacheUtil.TOP_SLIP_SAMPLE_SEGMENTED_STATE);
        int nowSelect = viewChannel.getSelectChannel();
        int channelSelect = CacheUtil.get().getInt(CacheUtil.TOP_SLIP_SAVE_CHANNEL_SELECT);
        if (channelSelect != nowSelect) {
            channelSelect = nowSelect;
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_SAVE_CHANNEL_SELECT, String.valueOf(channelSelect));
        }
        if (ChannelFactory.isDynamicCh(channelSelect) && segmentState == 0) {
            if (cbAllSegments.getVisibility() != View.VISIBLE) {
                change = true;
            }
            cbAllSegments.setVisibility(View.VISIBLE);
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_SAVE_ALL_SEGMENT_VISIBLE, String.valueOf(true), true);
        } else {
            if (cbAllSegments.getVisibility() != View.GONE) {
                change = true;
            }
            cbAllSegments.setVisibility(View.GONE);
            CacheUtil.get().putMap(CacheUtil.TOP_SLIP_SAVE_ALL_SEGMENT_VISIBLE, String.valueOf(false), true);
    ...
    TopViewChannel.onItemClickListener onChannelItemClickListener = new TopViewChannel.onItemClickListener() {
        ...
        public void checkChanged(int viewId, int checkedIndex, RadioButton radioButton) {
            ...
            boolean sendMsg = setAllSegmentsVisible();
            if (!sendMsg) {
                sendMsgSaveSegments();
            }
        }
...
//                        Command.get().getStorage().Save_Filename(saveNameEdit.getText().toString(), false);
                        ...
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_BIN, result);
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_BIN, "000");
                    }
                });
            }
        }
...
//    }
    ...
    private CompoundButton.OnCheckedChangeListener onCheckBoxChangedListener = new CompoundButton.OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if (buttonView.getId() == cbAllSegments.getId()) {
                CacheUtil.get().putMap(CacheUtil.TOP_SLIP_SAVE_ALL_SEGMENT_CHECK, String.valueOf(isChecked));
                Command.get().getStorage().Save_ALLSegments(isChecked, false);
                if (isChecked) {
                    cbAllSegments.setTextColor(getResources().getColor(R.color.color_Text_white));
                } else {
                    cbAllSegments.setTextColor(getResources().getColor(R.color.textColorNewTopViewEnable));
                }
//                int channelSelect = CacheUtil.get().getInt(CacheUtil.TOP_SLIP_SAVE_CHANNEL_SELECT);
//                setSaveTypeEnable(channelSelect);
                sendMsgSaveSegments();
            } else if (buttonView.getId() == checkFileNameAdd.getId()) {
                CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_BIN, String.valueOf(isChecked));
                txtSuffixNum.setEnabled(isChecked);
                RxBus.getInstance().post(RxEnum.MQ_MSG_SAVE_OR_INVOKE_SUFFIX_NUM_UPDATE, CacheUtil.WAVE_TYPE_BIN + CacheUtil.WAVE_STORE_PATH_SLIP + isChecked);
            }
        }
    };

    private void sendMsgSaveSegments() {
        msgSaveSegments.setVisibleSegments(cbAllSegments.getVisibility() == View.VISIBLE);
...
//        msgSaveSegments.setEnableTypeBin(rgSaveType.isEnabled(2));
        RxBus.getInstance().post(RxEnum.TOPSLIP_SAVE_SEGMENT, msgSaveSegments);
    }
...
//    };
    ...
    private View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == btnSave.getId()) {
                ...
                String filePath = spinner.getSelectItem() + File.separator + finalInput + ".bin";
    ...
    private void doSaveWave(String filePath) {
        ...
        if (ChannelFactory.isMathCh(ch) || ChannelFactory.isRefCh(ch)) {
        ...
        SaveManage.getInstance().allSaveEntrance(ch, 2, spinner.getSelectItem(), finalInput, null, new SaveManage.SaveCallBack() {
            @Override
...
//        Command.get().getStorage().Save_Filename(finalInput, false);
    ...
    private void onTextListener(String text) {
        ...
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_BIN, text);
    }
    ...
    TopViewSpinner.onItemSelectListener onItemSelectListener = new TopViewSpinner.onItemSelectListener() {
        ...
        public void onItemSelected(FileBean str) {
            CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_BIN, str.getPath());
