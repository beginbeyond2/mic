...
/*
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  模块定位：CAN串口触发RemoteId详情Fragment                                       ║
 * ║  核心职责：配置CAN协议远程帧ID触发条件（ID编辑）                                    ║
 * ║  架构设计：继承BaseDetail，实现CAN RemoteId的具体触发逻辑                           ║
 * ║  数据流向：右侧菜单 → setConsumer → 更新ID → sendMsg → FPGA命令                   ║
 * ║  依赖关系：BaseDetail / CanBus / Command / CacheUtil / SerialsDetailCanRemoteId  ║
 * ║  使用场景：CAN触发模式下远程帧ID子项的触发条件配置界面                                ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
import com.micsig.tbook.tbookscope.top.layout.trigger.serials.bean.SerialsDetailCanRemoteId; // 导入CAN RemoteId详情数据Bean
 ...
 *
 * CAN串口触发RemoteId详情Fragment。
 * 负责配置CAN协议中远程帧ID的触发条件（16进制ID编辑），
 * 并将配置同步到FPGA命令和CanBus总线。
 */

public class TopLayoutTriggerSerialsCanRemoteId extends TopLayoutTriggerSerialsBaseDetail { // CAN RemoteId触发详情Fragment，继承基类
    private TopViewEdit canRemoteIdEdit; // CAN远程帧ID编辑框
    private SerialsDetailCanRemoteId msgCanRemoteId; // CAN RemoteId详情数据Bean
    ...
    protected void initView(View view) { // 初始化界面控件
        canRemoteIdEdit = (TopViewEdit) view.findViewById(R.id.canRemoteIdEdit); // 查找CAN远程帧ID编辑框控件
        canRemoteIdEdit.setOnClickEditListener(onClickEditListener); // 设置编辑框点击监听器
        msgCanRemoteId = new SerialsDetailCanRemoteId(); // 创建CAN RemoteId详情数据Bean实例
        msgCanRemoteId.setCanRemoteIdEdit(DIGITS_16, canRemoteIdEdit.getText()); // 设置初始ID值（16进制）
        msgCanRemoteId.setCanRemoteIdEditTitle(canRemoteIdEdit.getHead()); // 设置ID编辑标题
    }

    /**
     * 获取布局资源ID。
     * @return CAN RemoteId布局资源ID
     */
    @Override // 重写基类的getLayoutResId抽象方法
    protected int getLayoutResId() { // 返回布局资源ID
        return R.layout.layout_triggerserials_canremoteid; // 返回CAN RemoteId布局
    }

    /**
     * 获取串口详情数据，从缓存中读取最新的ID值。
     * @param detailFlag 详情标志位
     * @return CAN RemoteId详情数据对象
     */
    @Override // 重写基类的getSerialsDetail抽象方法
    public ISerialsDetail getSerialsDetail(int detailFlag) { // 获取串口详情数据
        String remoteId = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_REMOTEID + getSerialsNumber()); // 从缓存读取ID值
        msgCanRemoteId.setCanRemoteIdEdit(DIGITS_16, remoteId); // 更新Bean中的ID值
        return msgCanRemoteId; // 返回详情数据Bean
    }

    /**
     * 处理右侧菜单串口消息，当CAN类型且选中RemoteId时发送消息。
    ...
    protected void setConsumer(RightMsgSerials rightMsgSerials) { // 处理右侧菜单消息
        if (rightMsgSerials.isSerials1() && serialsNumber==CacheUtil.S1 // 判断是否为串口1且编号匹配
            ...
            if (CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN + getSerialsNumber()) == 1) { // 判断CAN子项是否选中RemoteId
                //当且仅当当前can列表选中的是该项时，才向外发送消息
                sendMsg(msgCanRemoteId, rightMsgSerials.isFromEventBus()); // 发送消息通知外部
            }
        }
    }
    ...
    protected void setCache() { // 加载缓存数据
        String remoteId = CacheUtil.get().getString(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_REMOTEID + getSerialsNumber()); // 从缓存读取ID值
        canRemoteIdEdit.setText(remoteId); // 设置编辑框文本
        ...
        canBus.setFrameId(CanBus.CAN_TRIGGER_REMOTE_FRAME_ID, toD(remoteId, 16)); // 设置CAN远程帧ID

        msgCanRemoteId.setCanRemoteIdEdit(DIGITS_16, remoteId); // 更新Bean ID值
        ...
        if (((triggerIndex == TopLayoutTrigger.DETAIL_S1 && serialsNumber==CacheUtil.S1) // 判断S1是否匹配
                ...
                && CacheUtil.get().getInt(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN + getSerialsNumber()) == 1) { // 判断CAN子项是否为RemoteId
            sendMsg(msgCanRemoteId, false); // 发送消息通知外部
        }
    }
    ...
    public void setCommandData(int id, boolean isFromEventBus) { // 设置命令数据
        ...
        if (!canRemoteIdEdit.getText().equals(sId)) { // 如果编辑框文本与转换后ID不一致
            onTextListener(sId, isFromEventBus); // 触发文本变更处理
        }
    }

    /**
     * 处理单选组条件变更事件（CAN RemoteId无条件选择，空实现）。
     * @param view 单选组控件
     * @param item 选中的通道项
     */
    @Override // 重写基类的setOnCheckChangedListener抽象方法
    protected void setOnCheckChangedListener(TopViewRadioGroup view, TopBeanChannel item) { // 条件变更处理（CAN RemoteId无条件选择）
    ...
    protected void setOnClickEditListener(final TopViewEdit v, String text) { // 编辑框点击处理
        ...
        switch (v.getId()) { // 根据编辑框ID分支处理
            case R.id.canRemoteIdEdit: // CAN远程帧ID编辑框
                //该为16进制,8位限制
                dialogKeyBoard.setDecimalData(8, DIGITS_16, onDismissListener); // 设置键盘参数：8位16进制
                break; // 跳出switch
        }
    }
     ...
     */
    TopDialogNumberKeyBoard.OnDismissListener onDismissListener = new TopDialogNumberKeyBoard.OnDismissListener() { // 键盘关闭监听器
     ...
     * @param result 新的ID文本值
     * @param isFromEventBus 是否来自EventBus事件
     */
    private void onTextListener(String result, boolean isFromEventBus) { // 文本变更处理
        ...
        int val = (int) toDLong(result, IDigits.DIGITS_16); // 将16进制文本转换为10进制整数
        result = SerialsUtils.getHexBinFromInt(val, 8, IDigits.DIGITS_16); // 重新格式化为8位16进制字符串
        //Logger.i("onTextListener:2:" + result);

        CacheUtil.get().putMap(CacheUtil.TOP_SLIP_TRIGGER_SERIALS_CAN_REMOTEID + getSerialsNumber(), result); // 保存ID到缓存
        canRemoteIdEdit.setEdit(result); // 更新编辑框显示
        msgCanRemoteId.setCanRemoteIdEdit(DIGITS_16, result); // 更新Bean ID值
        sendMsg(msgCanRemoteId, isFromEventBus); // 发送消息通知外部
        Command.get().getTrigger_can().setType(cmdCh, 1, toD(result, IDigits.DIGITS_16), 0, 0, false); // 发送FPGA CAN RemoteId触发命令

        if (!isFromEventBus) { // 非EventBus事件时同步总线
            SerialChannel serialChannel = ChannelFactory.getSerialChannel(fpgaCh); // 获取串口通道实例
            if(serialChannel == null) return; // 通道为空则返回
            CanBus canBus = (CanBus) serialChannel.getBus(IBus.CAN); // 获取CAN总线实例
            canBus.setFrameId(CanBus.CAN_TRIGGER_REMOTE_FRAME_ID, toD(result, IDigits.DIGITS_16)); // 设置CAN远程帧ID
        }
    }
}
