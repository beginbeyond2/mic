...
public class CanvasGL implements ICanvasGL {
    ...
    private WeakHashMap<Bitmap, BasicTexture> bitmapTextureMap = new WeakHashMap<>();
    ...
    private ITextureRefresh textureRefresh;
    public CanvasGL(ITextureRefresh textureRefresh) {
        this(new GLES20Canvas());
        this.textureRefresh = textureRefresh;
    }

    long ts = 0;
    @Override
    public void onRefreshTexture() {
        long s = SystemClock.elapsedRealtime();
        if(s - ts >= DELAY_TIMES){
            ts = s;
            if(textureRefresh != null){
                textureRefresh.onRefresh();
            }
        }else{
            if(!mHandler.hasMessages(DELAY_REFRESHTEXTURE)){
                mHandler.sendEmptyMessageDelayed(DELAY_REFRESHTEXTURE,DELAY_TIMES-(s-ts));
            }
        }
    }
    private static final int DELAY_TIMES = 30;
    private static final int DELAY_REFRESHTEXTURE = 0x1110;
    private Handler mHandler = new Handler(Looper.getMainLooper()){
        @Override
        public void handleMessage(@NonNull Message msg) {
            super.handleMessage(msg);
            if(msg.what == DELAY_REFRESHTEXTURE){
                onRefreshTexture();
            }
        }
    };
    ...
    public void drawBitmap(Bitmap bitmap, RectF src, RectF dst) {
        drawBitmap(bitmap, src, dst, basicTextureFilter);
    }
    ...
    @Override
    public void invalidateTextureContent(Bitmap bitmap,Bitmap old) {
        freeTextureFromMap(old);
        BasicTexture resultTexture = getTextureFromMap(bitmap);
    ...
    private void freeTextureFromMap(Bitmap bitmap){
        if(bitmap != null) {
            BasicTexture resultTexture = bitmapTextureMap.remove(bitmap);
            if(!bitmap.isRecycled()){
                bitmap.recycle();;
            }
            if(resultTexture != null
                    && !resultTexture.isRecycled()){
                resultTexture.recycle();
            }
        }
    }
