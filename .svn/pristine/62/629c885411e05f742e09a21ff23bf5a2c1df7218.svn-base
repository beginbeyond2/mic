...
public class TopLayoutSaveCsv extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.layout_save_csv, container, false);
    }
    ...
    private void initView(View view) {
        viewChannel = view.findViewById(R.id.chanSaveCsv);
    ...
    public void savePathToCache() {
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_CSV,
                StrUtil.getStringFromList(FileBeanToStr.getDisPlayStrList(pathSet.getPositiveList()), CacheUtil.WAVE_STORE_PATH_SLIP));

        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_CSV,
                StrUtil.getStringFromList(FileBeanToStr.getAbsoluteStrList(pathSet.getPositiveList()), CacheUtil.WAVE_STORE_PATH_SLIP));

        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV, spinner.getSelectItem());
    }
...
//        RxBus.getInstance().getObservable(RxEnum.MAINBOTTOM_QUICKSAVE).subscribe(consumerMainBottomQuickSave);
        EventFactory.addEventObserver(EventFactory.EVENT_SAVECSV_RUN, eventSaveCsvObserver);
...
//        setChannelShow();
    ...
    private void setCache() {
        ...
        String waveName = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_CSV);
        if (waveName.isEmpty()) {
...
//        Command.get().getStorage().Save_ALLSegments(false, false);
        ...
        boolean isFileNumAddCheck = CacheUtil.get().getOtherBoolean(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_CSV);
        String suffixNum = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV);
    ...
    private void restorePath() {
        pathSet.clear();
        String pathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH + CacheUtil.WAVE_TYPE_CSV);
        String abPathCacheStr = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_ABSOLUTE_PATH + CacheUtil.WAVE_TYPE_CSV);
        String currentPath = CacheUtil.get().getOtherString(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV);
...
//                        Command.get().getStorage().Save_Filename(saveNameEdit.getText().toString(), false);
                        ...
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_NAME + CacheUtil.WAVE_TYPE_CSV, result);
                        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV, "000");
                    }
                });
            }
        }
    ...
    private CompoundButton.OnCheckedChangeListener onCheckBoxChangedListener = new CompoundButton.OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if (buttonView.getId() == checkFileNameAdd.getId()) {
                CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK + CacheUtil.WAVE_TYPE_CSV, String.valueOf(isChecked));
                txtSuffixNum.setEnabled(isChecked);
                RxBus.getInstance().post(RxEnum.MQ_MSG_SAVE_OR_INVOKE_SUFFIX_NUM_UPDATE, CacheUtil.WAVE_TYPE_CSV + CacheUtil.WAVE_STORE_PATH_SLIP + isChecked);
            }
        }
    };
    ...
    private final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == btnSave.getId()) {
                ...
                String filePath = spinner.getSelectItem() + File.separator + finalInput + ".csv";
    ...
    private void doSaveWave(String filePath) {
        ...
        SaveManage.getInstance().allSaveEntrance(ch, 1, spinner.getSelectItem(), finalInput, selectList, new SaveManage.SaveCallBack() {
            @Override
...
//        }
        txtSuffixNum.setText(text);
        CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_SUFFIX_CHECK_NUM + CacheUtil.WAVE_TYPE_CSV, text);
    }
    ...
    TopViewSpinner.onItemSelectListener onItemSelectListener = new TopViewSpinner.onItemSelectListener() {
        ...
        public void onItemSelected(FileBean str) {
            CacheUtil.get().putOtherMap(CacheUtil.TOP_SLIP_SAVE_WAVE_PATH_CURRENT + CacheUtil.WAVE_TYPE_CSV, str.getPath());
            addPathToPathSet(str);
        }
    };

    EventUIObserver eventSaveCsvObserver = new EventUIObserver() {
        @Override
        public void update(Object data) {
            ...
            int progress = 0;
            switch (base.getId()) {
                case EventFactory.EVENT_SAVECSV_RUN:
