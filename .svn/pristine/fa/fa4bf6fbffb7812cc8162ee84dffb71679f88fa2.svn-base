...
public class TopViewBandWidthHzLarge extends View {
//    private static final String[] RULERNAME = {"0", "1us", "10us", "100us", "1ms", "10ms", "100ms", "1s", "10s"};
    private static final String[] RULERNAME = {"10", "100", "1k", "10k", "100k", "1M", "10M", "100M", "1G"};
    ...
    public TopViewBandWidthHzLarge(Context context) {
        this(context, null);
    }

    public TopViewBandWidthHzLarge(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TopViewBandWidthHzLarge(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void onListener() {
        if (onRulerChangedListener != null) {
            ...
            if (px < 0) {
                ...
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 0, 50, 0), TopUtilBandWidthHz.UNIT_NS, 100);
            } else if (px < twoLineInterval * 19) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 10, 0.5, 1), TopUtilBandWidthHz.UNIT_US, 1);
            } else if (px < twoLineInterval * 28) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 19, 5, 10), TopUtilBandWidthHz.UNIT_US, 10);
            } else if (px < twoLineInterval * 37) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 28, 50, 100), TopUtilBandWidthHz.UNIT_US, 100);
            } else if (px < twoLineInterval * 46) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 37, 0.5, 1), TopUtilBandWidthHz.UNIT_MS, 1);
            } else if (px < twoLineInterval * 55) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 46, 5, 10), TopUtilBandWidthHz.UNIT_MS, 10);
            } else if (px < twoLineInterval * 64) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 55, 50, 100), TopUtilBandWidthHz.UNIT_MS, 100);
            } else if (px <= twoLineInterval * 73) {
                onRulerChangedListener.rulerChanged(getValueFromPx(px, 64, 0.5, 1), TopUtilBandWidthHz.UNIT_S, 1);
            }
        }
    }
    ...
    public void setValue(double curValue, String itemUnit, double itemValue) {
        float px = 0;
        if (TopUtilBandWidthHz.UNIT_NS.equals(itemUnit) && itemValue == 100) {
            px = getPxFromValue(curValue, 0, 50, 0);
        } else if (TopUtilBandWidthHz.UNIT_US.equals(itemUnit) && itemValue == 1) {
            px = getPxFromValue(curValue, 1, 0.5, 10);
        } else if (TopUtilBandWidthHz.UNIT_US.equals(itemUnit) && itemValue == 10) {
            px = getPxFromValue(curValue, 10, 5, 19);
        } else if (TopUtilBandWidthHz.UNIT_US.equals(itemUnit) && itemValue == 100) {
            px = getPxFromValue(curValue, 100, 50, 28);
        } else if (TopUtilBandWidthHz.UNIT_MS.equals(itemUnit) && itemValue == 1) {
            px = getPxFromValue(curValue, 1, 0.5, 37);
        } else if (TopUtilBandWidthHz.UNIT_MS.equals(itemUnit) && itemValue == 10) {
            px = getPxFromValue(curValue, 10, 5, 46);
        } else if (TopUtilBandWidthHz.UNIT_MS.equals(itemUnit) && itemValue == 100) {
            px = getPxFromValue(curValue, 100, 50, 55);
        } else if (TopUtilBandWidthHz.UNIT_S.equals(itemUnit) && itemValue == 1) {
...
//        }
        if (minNs < TopUtilBandWidthHz.TIME_US2NS) {//最小值小于1us
            long minPointX = (long) (minNs / 50 * (twoLineInterval / 2));
            pointX = Math.max(willPointX, minPointX);
        }else if (minNs < TopUtilBandWidthHz.TIME_US2NS*10){
            long minPointX =(long) getPxFromValue(minNs/1e3f, 1, 0.5, 10);
            pointX = Math.max(willPointX, minPointX);
        }else if (minNs < TopUtilBandWidthHz.TIME_US2NS*100){
            long minPointX = (long) getPxFromValue(minNs/1e4f, 10, 5, 19);
            pointX = Math.max(willPointX, minPointX);
        }else if (minNs < TopUtilBandWidthHz.TIME_US2NS*1e3){
            long minPointX = (long) getPxFromValue(minNs/1e5f, 100, 50, 28);
            pointX = Math.max(willPointX, minPointX);
        }else if (minNs < TopUtilBandWidthHz.TIME_US2NS*1e4){
            long minPointX = (long) getPxFromValue(minNs/1e6f, 1, 0.5, 37);
            pointX = Math.max(willPointX, minPointX);
        }

        if (maxNs >= TopUtilBandWidthHz.TIME_S2NS) {//最大值大于1s
            long maxPointX = (long) ((maxNs / (TopUtilBandWidthHz.TIME_S2NS / 2) - 2 + 64 * 2) * (twoLineInterval / 2));
