...
public class MainAdapterCenterSerialsWordSpi extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new Holder(LayoutInflater.from(context).inflate(R.layout.item_serialsword_spi, parent, false));
    }
    ...
    class Holder extends RecyclerView.ViewHolder {
        private SerialsWordSpiSingleRowTextView spiTextView;

        public Holder(View itemView) {
            super(itemView);
            spiTextView = (SerialsWordSpiSingleRowTextView) itemView.findViewById(R.id.bean);
        }


        public void bind(SerialBusTxtStruct.SpiStruct bean) {
            ViewGroup.LayoutParams layoutParams = spiTextView.getLayoutParams();
            layoutParams.height = formHeightDetail * ((bean.Data.trim().length() - 1) / ISerialsWord.MAXCHAR_EACHROW_DATA_SPI + 1);
            spiTextView.setLayoutParams(layoutParams);
            spiTextView.setData(bean, showMs);
        }
    }
}
