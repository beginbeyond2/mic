...
public class TopUtilBandWidthHz {
    ...
    public static final long DEFAULT_MIN_TIME = 300;//单位ns，30Hz
    public static final long DEFAULT_MAX_TIME = 10 * TIME_S2NS;//单位ns，1GHz
...
//            }
    ...
    /**
     * 根据top trigger页面的时间框对比置换right channel页面的hz
     */
    public static String getHzFromS(String s) {
        long ns = TBookUtil.get_nsFromTime(s);
        return TBookUtil.getHz3FromHz(1.0 * ns / 10);
    }

    /**
     * 根据top trigger页面的时间框对比置换right channel页面的hz
     */
    public static String getSFromHz(String hz) {
        long hzInt = TBookUtil.getHzFromHz3(hz);
        return TBookUtil.getTime3FromPs(hzInt * 10 * 1000 * 10);//hz和ns的换算10，0.1ps和ns的转换1000*10...
    }
}
